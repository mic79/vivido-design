...
(function () {
  ...
  AFRAME.registerComponent('vr-menu', {
    ...
    toggleMenu: function () {
      ...
      var vlg = this.el.components && this.el.components['vrknockout-game'];
    ...
    handleMenuClick: function (buttonId) {
      switch (buttonId) {
        ...
        case 'menu-vk-track-prev': this.vkMenuCourseNav(-1); break;
        case 'menu-vk-track-next': this.vkMenuCourseNav(1); break;
    ...
    setMode: function (mode) {
      ...
      var courseSection = document.getElementById('menu-vk-course-section');
      if (courseSection) {
        var vkScM = document.querySelector('a-scene[vrknockout-game]');
        var hasVkM = !!(vkScM && vkScM.components && vkScM.components['vrknockout-game']);
        courseSection.setAttribute('visible', hasVkM && mode !== 'stats');
      }
    ...
    vkMenuCourseNav: function (delta) {
      var vkScene = document.querySelector('a-scene[vrknockout-game]');
      var vkG = vkScene && vkScene.components && vkScene.components['vrknockout-game'];
      if (!vkG || typeof vkG.vkCycleMenuCourse !== 'function') return;
      if (window.isMultiplayer && !vkG.isHost) return;
      vkG.vkCycleMenuCourse(delta);
      this.updateMenuDisplay();
    },
    ...
    toggleAutoRoll: function () {
      ...
      try {
        localStorage.setItem('vrknockout-auto-stabilize', window._vlAutoRollEnabled ? '1' : '0');
      } catch (e) {}
      ...
      if (txt) {
        txt.setAttribute('text', 'value', window._vlAutoRollEnabled ? 'Upright assist ON' : 'Upright assist OFF');
      }
    },
    ...
    toggleMusic: function () {
      ...
      if (window._musicEnabled) {
        ...
        var vkSceneM = document.querySelector('a-scene[vrknockout-game]');
        var vkGm = vkSceneM && vkSceneM.components && vkSceneM.components['vrknockout-game'];
        if (vkGm && vkGm.vkMatchActive) inMatch = true;
        if (inMatch) {
          if (vkGm && vkGm.vkMatchActive) {
            if (mm && mm.components.sound) mm.components.sound.stopSound();
            if (bgm && bgm.components.sound && sm) {
              if (!window._bgMusicStarted) {
                bgm.components.sound.playSound();
                window._bgMusicStarted = true;
                sm._setVolume(bgm, 0);
              }
              sm._fadeSound(bgm, sm.bgMusicVolume, sm.matchMusicVolume, 500);
            }
          } else if (mm && mm.components.sound) {
    ...
    startSingleplayerMatch: function () {
      ...
      if (gmEl && gmEl.components['game-manager']) {
        ...
        var vlRoot = document.querySelector('a-scene[vrknockout-game]') || document.querySelector('[vrknockout-game]');
        var vlc = vlRoot && vlRoot.components && vlRoot.components['vrknockout-game'];
        if (vlc && typeof vlc.vkToggleMatchFromMenu === 'function') {
          vlc.vkToggleMatchFromMenu();
    ...
    updateMenuDisplay: function () {
      ...
      if (typeof window._vlAutoRollEnabled === 'undefined') {
        try {
          var r0 = localStorage.getItem('vrknockout-auto-stabilize');
          if (r0 === null || r0 === '') {
            window._vlAutoRollEnabled = localStorage.getItem('vrknockout-auto-yaw') !== '0';
          } else {
      ...
      if (arTxt) {
        arTxt.setAttribute('text', 'value', window._vlAutoRollEnabled ? 'Upright assist ON' : 'Upright assist OFF');
      }

      var vkSceneK = document.querySelector('a-scene[vrknockout-game]');
      var vkGK = vkSceneK && vkSceneK.components && vkSceneK.components['vrknockout-game'];
      var trName = document.getElementById('menu-vk-track-name');
      var trPrev = document.getElementById('menu-vk-track-prev');
      var trNext = document.getElementById('menu-vk-track-next');
      if (trName && vkGK) {
        var trk = vkGK._vkCourseTrack | 0;
        var label =
          trk === 3 ? '3 Sliding tiles' : trk === 2 ? '2 Spinners' : '1 Ramp + rocks';
        trName.setAttribute('text', 'value', label);
      }
      if (trPrev && trNext) {
        var hostNav = vkGK && (!window.isMultiplayer || vkGK.isHost);
        trPrev.setAttribute('visible', !!(vkGK && hostNav));
        trNext.setAttribute('visible', !!(vkGK && hostNav));
        if (vkGK && hostNav) {
          setButtonColor(trPrev, '#555555');
          setButtonColor(trNext, '#555555');
        }
      }
      var courseSectionUp = document.getElementById('menu-vk-course-section');
      if (courseSectionUp) {
        courseSectionUp.setAttribute('visible', !!(vkGK && this.menuMode !== 'stats'));
      }
      ...
      if (!ls) {
        ...
        if (startMatchBtn) {
          ...
          if (showStart) {
            ...
            var vkScene = document.querySelector('a-scene[vrknockout-game]');
            var vkG = vkScene && vkScene.components && vkScene.components['vrknockout-game'];
            var vkPlaying = !!(vkG && vkG.vkMatchActive);
            if (vkG && !gm) {
              isPlaying = vkPlaying;
            } else if (vkG && gm) {
              isPlaying = isPlaying || vkPlaying;
            }
            if (stxt) stxt.setAttribute('text', 'value', isPlaying ? 'STOP MATCH' : 'START MATCH');
      ...
      var vlSceneBtn = document.querySelector('a-scene[vrknockout-game]');
      var vlGameBtn = vlSceneBtn && vlSceneBtn.components['vrknockout-game'];
      if (startMatchBtn2 && startMatchText) {
        if (this.menuMode === 'stats') {
          ...
          startMatchText.setAttribute('text', 'value', vlPlaying ? 'STOP MATCH' : 'START MATCH');
          ...
          if (isMatchPlayer && ls.matchState === 'PLAYING') {
            startMatchBtn2.setAttribute('visible', true);
            startMatchText.setAttribute('text', 'value', 'STOP MATCH');
