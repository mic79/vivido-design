...
export function createHdrBvhLighting(envData, sceneBVH, params) {
  ...
  function findSunInHDR() {
    ...
    if (typeof console !== "undefined" && console.log) {
      ...
      console.log(
        `[brutalistVR7] BVH sun: centroid elevation ${(90 - angle).toFixed(1)}° from ${count} bright pixels above ${(SUN_REGION_THRESHOLD * 100).toFixed(1)}% of peak; jitter cone ±${(Math.atan(SUN_JITTER / 2) * 180 / Math.PI).toFixed(1)}°; avg luminance ${avgL.toFixed(3)}; path tracer: ${MAX_BOUNCES} bounces, albedo ${BOUNCE_ALBEDO.toFixed(2)}, terminator ${BOUNCE_ENABLED ? "skyFill" : "0"}`,
      );
    }
  }
