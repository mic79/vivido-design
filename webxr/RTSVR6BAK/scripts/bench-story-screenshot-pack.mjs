#!/usr/bin/env node
/**
 * Story mode factual pack: RTSVR4 / RTSVR6 / RTSVR5Forest
 ...
 *
 *   node RTSVR6/scripts/bench-story-screenshot-pack.mjs
 *
...
const OUT_ROOT = path.join(WEBXR, 'RTSVR6', 'bench-story-pack');
...
const PROJECTS = [
  ...
  { id: 'RTSVR6', root: path.join(WEBXR, 'RTSVR6'), port: 8785 },
  { id: 'rtsvr6Forest', root: path.join(WEBXR, 'rtsvr6Forest'), port: 8786 },
];
...
async function main() {
  ...
  const userShots = [
    ...
    },
    {
      ...
      name: 'user-link-RTSVR6-0.5.117-Story-120fps-d108-tK1216.jpg',
      note: 'User Meta Link VR · RTSVR6 0.5.117 · 120 FPS · d=108 tK=1216 · Shadows ON · MSAA off',
    },
    {
      src: path.join(
        process.env.USERPROFILE || '',
      ...
      name: 'user-link-RTSVR6-0.5.127-Story-120fps-d116-tK1313.jpg',
      note: 'User Meta Link VR · RTSVR6 0.5.127 · 120 FPS · d=116 tK=1313 · Shadows ON · MSAA off',
    },
  ];
  ...
  const pack = {
    ...
    method: {
      harness: 'RTSVR6/scripts/bench-story-screenshot-pack.mjs',
      mode: 'story',
  ...
  md += `| RTSVR6 | ~${pack.userQuestStandaloneReported.RTSVR6_fps} | Shadows/MSAA off |\n`;
