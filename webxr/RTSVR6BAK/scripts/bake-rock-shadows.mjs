...
const baked = await page.evaluate(
  async ({ glbUrl, hdrUrl, resPlate, resSkirt, shadowMapRes, dark, gameSun }) => {
    ...
    gltf.scene.traverse((obj) => {
      ...
      // Skip Cross/X barrier walls — at canyon scale they planar-darken ~90% of the plate.
      if (/^RTS_/i.test(n) || /light|camera|helper|grid/i.test(n) || /_Barrier_/i.test(n)) return;
      rocks.push(obj);
    });
