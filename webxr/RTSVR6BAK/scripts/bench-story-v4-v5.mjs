#!/usr/bin/env node
/**
 * Headless Story pose matrix: RTSVR4 vs RTSVR6 (same machine, same seed, same poses).
 *
 *   node RTSVR6/scripts/bench-story-v4-v5.mjs
 *
...
function printCompare(a, b) {
  ...
  console.log('RTSVR6', JSON.stringify(b.terrain, null, 2));
}
...
async function main() {
  ...
  const v5 = await benchProject('RTSVR6', path.join(WEBXR, 'RTSVR6'), PORT5, browser);
  await browser.close();

  printCompare(v4, v5);
  const outPath = path.join(WEBXR, 'RTSVR6', 'bench-story-v4-v5.json');
  fs.writeFileSync(
    outPath,
    JSON.stringify(
      {
