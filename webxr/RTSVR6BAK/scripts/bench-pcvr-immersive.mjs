 ...
 *
 *   node RTSVR6/scripts/bench-pcvr-immersive.mjs
 *
 * Prereq: headset on, Meta Link / OpenXR (or VD) connected.
 * Env: MODE=1v1|story SAMPLE_MS=4000 STORY_SEED=42 PROJECT=all|RTSVR6|...
 */
...
const ROOT5 = path.join(WEBXR, 'RTSVR6');
...
const ALL = [
  ...
  { id: 'RTSVR6', root: path.join(WEBXR, 'RTSVR6') },
  { id: 'rtsvr6Forest', root: path.join(WEBXR, 'rtsvr6Forest') },
];
const PROJECTS =
...
async function main() {
  ...
  const pack = {
    ...
    method: {
      harness: 'RTSVR6/scripts/bench-pcvr-immersive.mjs',
