/**
 * ChaseVR — VRrunner on-foot locomotion (grab/throw jump, slide, wingsuit, archery)
...
function ensureTriggerAudioBuffer() {
    ...
    triggerAudioLoadPromise = fetch(TRIGGER_SFX_URL)
        ...
        .catch(function(err) {
            ...
            console.warn('ChaseVR trigger SFX failed to load:', TRIGGER_SFX_URL, err);
            return null;
        });
...
function isInHelicopter() {
    return !!(window.__chasevrPlayerHelicopter && window.__chasevrPlayerHelicopter.isHelicopterMode());
}
...
function updateDoorVisibility() {
    ...
    const PH = window.__chasevrPlayerHelicopter;
