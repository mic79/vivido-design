...
import {
  rayCastRunnerGlbMeshes,
  runnerGlbMeshesIntersectsSphere,
  pushWorldPointOutOfRunnerGlbMeshes,
} from "./runnerLevel.js";
...
/** Optional (VRrunner): (box, point, normal) => true if runner breakable glass consumed hit */
let shatterRunnerGlassIfHit_ = null;
...
/** When set, left X only cycles arrow type if this returns true (ChaseVR: foot mode). */
let canToggleArrowTypeOnX_ = null;
...
/** Resolve ChaseVR/audio/ from this module (not page-relative paths). */
function chasevrAudioUrl(filename) {
...
const MUSIC_FILES = {
  calm: chasevrAudioUrl("alexgrohl-the-futuristic-ambience-everything-is-one-179395.mp3"),
  /* Two battle tracks — pick one randomly per session for variety. */
  battle: [
    chasevrAudioUrl("amaksi-clanabogan-beast-phonk-148615.mp3"),
    chasevrAudioUrl("amaksi-unleashed-fury-173854.mp3"),
  ],
};
...
function pointInsideAnyOBB(point, pad = 0) {
  ...
  if (runnerGlbMeshesIntersectsSphere(point, pad)) return true;
  return false;
}
...
function pushDroneOutOfWalls(drone) {
  ...
  for (let iter = 0; iter < 3; iter++) {
    ...
    _glbDronePushAcc.set(0, 0, 0);
    if (pushWorldPointOutOfRunnerGlbMeshes(p, PAD, _glbDronePushAcc)) {
      pushed = true;
      const n = _glbDronePushAcc.lengthSq();
      if (n > 1e-8) {
        const vDot = drone.velocity.dot(_glbDronePushAcc);
        if (vDot < 0) drone.velocity.addScaledVector(_glbDronePushAcc, -vDot / n);
      }
    }
    if (!pushed) break;
  }
...
function rayHitWorld(origin, dir, maxDist = 200) {
  ...
  const glbT = rayCastRunnerGlbMeshes(origin, dir, maxDist, null);
  return Math.min(best, glbT);
}
...
function rayHitWorldRich(origin, dir, maxDist, outHit) {
  ...
  const glbT = rayCastRunnerGlbMeshes(origin, dir, maxDist, _glbWorldRayHit);
  if (Number.isFinite(glbT) && glbT < best) {
    outHit.t = glbT;
...
function _updateGrenadeAimPreview() {
  ...
  for (let i = 0; i < GRENADE_TRAJ_SAMPLES; i++) {
    ...
    if (_grenadeAimSimPos.y <= 0.05 && prevY > 0.05) {
      ...
      if (!impactPos && runnerGlbMeshesIntersectsSphere(_grenadeAimSimPos, 0.12)) {
        impactPos = new THREE.Vector3(prevX, prevY, prevZ);
      }
    }
  }
 ...
 * visible arrow mesh.
 *
 * The bow-hand trigger: rising edge detonates a **mid-flight explosive**;
 * **held** while a **grapple** hook is stuck accelerates the player along the
 * rope (rig velocity); a cyan line runs bow-aim → arrow tip in flight and
 * while the hook is winchable. The first bow-hand **release** after you have
 * pulled with the winch hides the line and disables further winches on that
 * stuck arrow (momentum is retained in physics).
 * VR-only — there is no desktop archery path. */
...
/** World / drone stick: place impact on the shaft midpoint so ~½ the arrow stays outside. */
const ARROW_STICK_SURFACE_AT_MIDPOINT = ARROW_LENGTH * 0.5;
...
/** When false the bow stays stowed (ChaseVR holster); VRrunner keeps this true by default. */
let bowEquipped_ = true;
let bowGroup_ = null;
...
let bowHandTriggerHeld_ = false;
/** Previous-frame bow-hand trigger (for winch release edge). */
let prevBowHandWinch_ = false;
/** Stuck grapple hook mesh; bow-hand trigger pulls via velocity until released once. */
let activeGrappleMesh_ = null;
/** In-flight or stuck grapple arrow root — bow→tip rope follows this mesh. */
let grappleRopeTargetMesh_ = null;
/** True after we've applied at least one winch impulse this hook; release then consumes the hook. */
let grappleWinchEngagedThisHook_ = false;
/** Line geometry bow → grapple tip (cyan). */
let grappleRopeLine_ = null;
const _grappleWinchTo = new THREE.Vector3();
const _grappleWinchDir = new THREE.Vector3();
const _grappleTipWorld = new THREE.Vector3();
const _grappleFwdWorld = new THREE.Vector3();
...
function _getControllerByHand(hand) {
  ...
  /* Prefer grips — ChaseVR (and VRrunner) parent visible hands here; grip
   * pose matches what the player sees. Target-ray controllers may never
   * receive handedness if initBots runs after the connected event. */
  for (let i = 0; i < 4; i++) {
    const grip = renderer_.xr.getControllerGrip(i);
    if (!grip) continue;
    const gh =
      grip.userData?.handedness ||
      grip.userData?.xrInputSource?.handedness;
    if (gh === hand) return grip;
  }
  for (let i = 0; i < 4; i++) {
...
function _buildArrowMesh(isExplosive = false, isGrapple = false) {
  const r = _ensureArrowResources();
  const root = new THREE.Group();
  root.userData.kind = "arrow";
  root.userData.sharedRes = true;
  root.userData.explosive = !!isExplosive && !isGrapple;
  root.userData.grapple = !!isGrapple;
  ...
  /* Head — cone | explosive sphere | cyan grapple claw. */
  let head;
  if (isGrapple) {
    head = _buildGrappleArrowHead();
  } else if (isExplosive) {
...
function _attachBowToHand(hand) {
  ...
  let ctrl = _getControllerByHand(hand);
  if (!ctrl) {
    _syncControllerHandednessFromSession();
    ctrl = _getControllerByHand(hand);
  }
  ...
  bowGroup_.rotation.set(-Math.PI / 2, Math.PI, 0);
...
function _startDraw() {
   ...
   * than an empty bow). */
  const wantGrapple = arrowType_ === ARROW_TYPE_GRAPPLE;
  const wantExplosive = !wantGrapple && arrowType_ === ARROW_TYPE_EXPLOSIVE && _explosiveReady();
  ...
  nockedArrow_ = _buildArrowMesh(wantExplosive, wantGrapple);
...
function _releaseDraw() {
  ...
  const isGrapple = nockedArrow_?.userData?.grapple === true;
  /* Spawn from the bow's world position so the arrow visually leaves
   * the bow front, not the draw hand. */
  _spawnFlightArrow(_bowWorld, _arrowDir, speed, isExplosive, isGrapple);
...
function _spawnFlightArrow(origin, dir, speed, isExplosive = false, isGrapple = false) {
  if (isGrapple) _disarmGrapple();
  const mesh = _buildArrowMesh(isExplosive, isGrapple);
  ...
  scene_.add(mesh);
  if (isGrapple) grappleRopeTargetMesh_ = mesh;
  arrows_.push({
    mesh,
...
const GRAPPLE_WINCH_ACCEL = 44;
const GRAPPLE_MAX_WINCH_SPEED = 24;
const GRAPPLE_WINCH_STOP_DIST = 0.38;

function _grappleArrowTipWorld(mesh, out) {
  mesh.updateMatrixWorld(true);
  mesh.getWorldQuaternion(_q0);
  _grappleFwdWorld.set(0, 0, 1).applyQuaternion(_q0).normalize();
  mesh.getWorldPosition(out);
  out.addScaledVector(_grappleFwdWorld, ARROW_LENGTH * 0.42);
}

function _ensureGrappleRopeLine() {
  if (grappleRopeLine_ || !scene_) return;
  ...
  grappleRopeLine_ = new THREE.Line(geom, mat);
  grappleRopeLine_.frustumCulled = false;
  grappleRopeLine_.visible = false;
  scene_.add(grappleRopeLine_);
}

function _hideGrappleRope() {
  if (grappleRopeLine_) grappleRopeLine_.visible = false;
}

function _clearGrappleRopeIfMesh(mesh) {
  if (grappleRopeTargetMesh_ === mesh) {
    grappleRopeTargetMesh_ = null;
    _hideGrappleRope();
  }
}

function _updateGrappleRopeVisual() {
  _ensureGrappleRopeLine();
  if (!grappleRopeLine_ || !grappleRopeTargetMesh_ || !grappleRopeTargetMesh_.parent) {
    if (grappleRopeLine_) grappleRopeLine_.visible = false;
    return;
  }
  if (grappleRopeTargetMesh_.userData.grappleWinchUsed) {
    grappleRopeLine_.visible = false;
    return;
  }
  if (!_getBowAimPos(_bowWorld)) {
    grappleRopeLine_.visible = false;
    return;
  }
  _grappleArrowTipWorld(grappleRopeTargetMesh_, _grappleTipWorld);
  const pos = grappleRopeLine_.geometry.attributes.position;
  pos.setXYZ(0, _bowWorld.x, _bowWorld.y, _bowWorld.z);
  pos.setXYZ(1, _grappleTipWorld.x, _grappleTipWorld.y, _grappleTipWorld.z);
  pos.needsUpdate = true;
  grappleRopeLine_.geometry.computeBoundingSphere();
  grappleRopeLine_.visible = true;
}

function _attachGrappleAnchor(mesh) {
  activeGrappleMesh_ = mesh;
  grappleWinchEngagedThisHook_ = false;
  if (mesh) {
    mesh.userData.grappleActive = true;
    mesh.userData.grappleWinchUsed = false;
  }
}

function _disarmGrapple() {
  _hideGrappleRope();
  grappleRopeTargetMesh_ = null;
  grappleWinchEngagedThisHook_ = false;
  if (!activeGrappleMesh_) return;
  const m = activeGrappleMesh_;
  activeGrappleMesh_ = null;
  m.userData.grappleActive = false;
  const idx = stuckArrows_.indexOf(m);
  if (idx >= 0) stuckArrows_.splice(idx, 1);
...
function _applyGrappleWinch(dt) {
  ...
  activeGrappleMesh_.updateMatrixWorld(true);
  _grappleArrowTipWorld(activeGrappleMesh_, _grappleWinchTo);
  _grappleWinchDir.copy(_grappleWinchTo).sub(cameraRig_.position);
  const dist = _grappleWinchDir.length();
  if (dist < GRAPPLE_WINCH_STOP_DIST) return;
  _grappleWinchDir.divideScalar(dist);
  const gv = getPlayerVelocity_?.();
  if (!gv) return;
  const along = gv.dot(_grappleWinchDir);
  const headroom = GRAPPLE_MAX_WINCH_SPEED - along;
  if (headroom <= 0) return;
  const dv = Math.min(GRAPPLE_WINCH_ACCEL * dt, headroom);
  gv.addScaledVector(_grappleWinchDir, dv);
  grappleWinchEngagedThisHook_ = true;
  pulseHapticActuator(bowHandedness_, 0.12, 14);
}

let _grappleHeadGeo = null;
let _grappleHeadMat = null;
function _ensureGrappleArrowHeadResources() {
  if (_grappleHeadGeo) return;
  _grappleHeadGeo = new THREE.TorusGeometry(0.014, 0.005, 8, 12, Math.PI * 1.35);
  _grappleHeadMat = new THREE.MeshStandardMaterial({
    color: 0x0a3048,
    emissive: 0x00c8d4,
...
function _buildGrappleArrowHead() {
  _ensureGrappleArrowHeadResources();
  const head = new THREE.Mesh(_grappleHeadGeo, _grappleHeadMat);
  head.rotation.x = Math.PI / 2;
  head.userData.explosive = false;
  head.userData.grapple = true;
  return head;
}
...
function updateArrows(dt) {
  ...
  /* First bow-hand trigger **release** after a winch pull → hide rope and
   * disable further winches on this hook (momentum is already in rigVelocity). */
  if (prevBowHandWinch_ && !bowHandTriggerHeld_ && activeGrappleMesh_ && grappleWinchEngagedThisHook_) {
    activeGrappleMesh_.userData.grappleWinchUsed = true;
    grappleWinchEngagedThisHook_ = false;
    _hideGrappleRope();
  }
  if (drawing_) {
  ...
  for (let i = arrows_.length - 1; i >= 0; i--) {
    ...
    if (segLen >= 1e-5) {
      ...
      if (a.explosive && (hasDroneOrAA || hasMissile || hasWallFinal || hasRagdoll || hasCombat)) {
        ...
        arrowRagdollExplosion_?.(
          _arrowTmp1,
          EXPLOSIVE_ARROW_BLAST_RADIUS,
          EXPLOSIVE_ARROW_PUSH,
        );
...
function _stickArrowOnDrone(arrow, hit, followMesh = null) {
  ...
  /* Flight +Z is into the target; pull the nock back so the hit lies ~mid-shaft
   * (half in, half out) instead of burying the whole arrow. */
  _arrowTmp1.set(0, 0, 1).applyQuaternion(arrow.mesh.quaternion);
  arrow.mesh.position.addScaledVector(_arrowTmp1, -ARROW_STICK_SURFACE_AT_MIDPOINT);
  ...
  if (arrow.grapple) _attachGrappleAnchor(arrow.mesh);
}

function _stickArrowOnWorld(arrow, point) {
  /* +Z is along flight into the wall; put the surface hit at mid-shaft so
   * roughly half the arrow stays visible outside the face. */
  _arrowTmp1.set(0, 0, 1).applyQuaternion(arrow.mesh.quaternion);
  arrow.mesh.position.copy(point).addScaledVector(_arrowTmp1, -ARROW_STICK_SURFACE_AT_MIDPOINT);
  _enqueueStuckArrow(arrow.mesh);
  if (arrow.grapple) _attachGrappleAnchor(arrow.mesh);
}
...
function _enqueueStuckArrow(mesh) {
  ...
  while (stuckArrows_.length > ARROW_MAX_STUCK) {
    ...
    if (oldest === activeGrappleMesh_) {
      activeGrappleMesh_ = null;
      oldest.userData.grappleActive = false;
    }
    if (oldest === grappleRopeTargetMesh_) {
      grappleRopeTargetMesh_ = null;
      _hideGrappleRope();
    }
...
/** VRrunner pit reset: remove in-flight + stuck arrows and cancel any draw. */
export function clearRunnerArcheryVolleys() {
  _disarmGrapple();
  prevBowHandWinch_ = false;
  for (let i = arrows_.length - 1; i >= 0; i--) {
    const a = arrows_[i];
    if (a?.mesh && scene_) scene_.remove(a.mesh);
    arrows_.splice(i, 1);
  }
  for (const m of stuckArrows_) {
    if (m?.parent) m.parent.remove(m);
    else if (scene_) scene_.remove(m);
  }
  stuckArrows_.length = 0;
  _cancelDraw();
  nockedArrow_ = null;
  archeryFireCooldown_ = 0;
  prevDrawTrigger_ = true;
}
...
/** Remove bow from hand; cancels draw / grenade aim. */
export function stowBow() {
  if (!ARCHERY_ENABLED) return;
  if (drawing_) _cancelDraw();
  if (grenadeAiming_) cancelGrenadeAim();
...
export function equipBowToHand(hand) {
  if (!ARCHERY_ENABLED || editorLocomotionActive_) return false;
  if (hand !== "left" && hand !== "right") return false;
  if (hand !== bowHandedness_) {
    if (drawing_) _cancelDraw();
    if (grenadeAiming_) cancelGrenadeAim();
    bowHandedness_ = hand;
    prevDrawTrigger_ = true;
...
/** Holster grab: stow if this hand already holds the bow, else equip here. Returns true when equipped. */
...
export function pulseControllerHaptic(handedness, intensity, durationMs) {
  pulseHapticActuator(handedness, intensity, durationMs);
}

/* ── Arrow types (normal / explosive / grapple) ─────────────────────────
 *
 * The bow nocks arrows of the *currently selected type*. Three types:
 *
 ...
 *                  background.
 *   "grapple"    — cyan torus hook; same damage/stick as normal on hit.
 *                  **While the bow-hand trigger is held**, the rig is
 *                  winched toward the stuck hook (see `_applyGrappleWinch`).
 *                  Firing a new grapple removes the previous hook.
 *
 * Toggle: X on the left controller cycles normal → explosive → grapple.
...
const ARROW_TYPE_GRAPPLE = "grapple";
...
export function setArrowType(t) {
  if (t !== ARROW_TYPE_NORMAL && t !== ARROW_TYPE_EXPLOSIVE && t !== ARROW_TYPE_GRAPPLE) {
    console.warn('setArrowType: pass "normal", "explosive", or "grapple"');
    return arrowType_;
  }
...
export function toggleArrowType() {
  if (arrowType_ === ARROW_TYPE_NORMAL) return setArrowType(ARROW_TYPE_EXPLOSIVE);
  if (arrowType_ === ARROW_TYPE_EXPLOSIVE) return setArrowType(ARROW_TYPE_GRAPPLE);
  return setArrowType(ARROW_TYPE_NORMAL);
}
...
function _swapNockedArrowVisual(parentCtrl) {
  ...
  const wantGrapple = arrowType_ === ARROW_TYPE_GRAPPLE;
  const wantExplosive = !wantGrapple && arrowType_ === ARROW_TYPE_EXPLOSIVE && _explosiveReady();
  const wasGrapple = oldHead.userData?.grapple === true;
  const wasExplosive = oldHead.userData?.explosive === true;
  const wantHeadKind = wantGrapple ? "grapple" : wantExplosive ? "explosive" : "normal";
  const haveHeadKind = wasGrapple ? "grapple" : wasExplosive ? "explosive" : "normal";
  if (wantHeadKind === haveHeadKind) {
    nockedArrow_.userData.explosive = wantExplosive;
    nockedArrow_.userData.grapple = wantGrapple;
    return;
  }
  const oldZ = oldHead.position.z;
  nockedArrow_.remove(oldHead);
  const newHead = wantGrapple
    ? _buildGrappleArrowHead()
    : wantExplosive
      ? _buildExplosiveArrowHead()
      : _buildNormalArrowHead();
...
/** Shared reset body: tear down combat actors, clear caches, respawn.
 * @param {boolean} rearmCombat — if true, turn combat back on (fresh wave-1); if false, leave combat off (VRrunner pit fall). */
function _resetGameCore(rearmCombat) {
  ...
  if (rearmCombat) {
    /* Full restart: combat on starts wave 1 fresh and resets player HP. */
    setBotsEnabled(true);
  }
...
/** When false, right-hand B (button[5]) is not used to toggle combat
 *  (VRrunner binds B to slide instead). Default true for other builds. */
let battleOnBEnabled_ = true;
/** VRrunner: right A swaps bow hand. ChaseVR foot: holster picks hand; A cycles arrow type. */
let bowHandToggleOnA_ = true;
export function setBattleOnBEnabled(v) {
  battleOnBEnabled_ = !!v;
}
export function setBowHandToggleOnAEnabled(v) {
  bowHandToggleOnA_ = !!v;
}
...
function _canToggleArrowTypeOnX() {
  if (!canToggleArrowTypeOnX_) return true;
  try {
    return !!canToggleArrowTypeOnX_();
  } catch (_) {
    return true;
  }
}
...
function pollVRInputs() {
  ...
  for (const src of session.inputSources) {
    ...
    if (src.handedness === "right") {
       ...
       * - B toggles combat when `battleOnBEnabled_` is true (default).
  ...
  /* B (right) toggles combat mode on rising edge (unless disabled). */
  if (battleOnBEnabled_ && battleButton && !prevButtonX_) setBotsEnabled(!enabled_);
  prevButtonX_ = battleButton;

  /* X (left) toggles arrow type on rising edge. The "X Switch arrow"
   * hint above the X button (left grip) advertises this. */
  if (xButton && !prevXButton_ && _canToggleArrowTypeOnX()) toggleArrowType();
  prevXButton_ = xButton;

  if (ARCHERY_ENABLED && bowEquipped_) {
    /* Lazily attach the bow as soon as a controller has reported its
     * handedness. Idempotent — no-op once attached. */
    _ensureArcherySetup();

    /* Right A: swap bow hand (VRrunner) or cycle arrow type (ChaseVR foot). */
    if (aButton && !prevAButton_) {
      if (bowHandToggleOnA_) _toggleBowHand();
      else toggleArrowType();
    }
    prevAButton_ = aButton;
    ...
    /* Grapple anchor + bow trigger held = winch (continuous). Skip explosive
     * detonate on rising edge while a hook is stuck so a stray tap doesn't
     * blow a mid-air warhead when the player meant to pull. */
    const grappleHooked = !!activeGrappleMesh_?.parent;
    if (bowHandTrigger && !prevBowHandTrig && _explosiveArrowInFlight() && !grappleHooked) {
      detonateExplosiveArrowInFlight();
    }
    bowHandTriggerHeld_ = !!bowHandTrigger;

    /* Persist raw per-hand trigger state for the prev-edge lookup
     * above on the next frame. */
    prevTriggerL_ = triggerL;
    prevTriggerR_ = triggerR;
  } else if (ARCHERY_ENABLED) {
    prevAButton_ = aButton;
  } else {
...
function attachHandednessTagging() {
  for (let i = 0; i < 4; i++) {
    const ctrl = renderer_.xr.getController(i);
    if (ctrl && !ctrl.userData.handednessHooked) {
    ...
    const grip = renderer_.xr.getControllerGrip(i);
    if (grip && !grip.userData.handednessHooked) {
      grip.userData.handednessHooked = true;
      grip.addEventListener("connected", (e) => {
        grip.userData.handedness = e?.data?.handedness || null;
        if (e?.data) grip.userData.xrInputSource = e.data;
      });
...
/** Back-fill handedness when bots init after controllers are already connected. */
function _syncControllerHandednessFromSession() {
  const session = renderer_?.xr?.getSession?.();
  if (!session?.inputSources) return;
  for (const src of session.inputSources) {
    ...
    for (let i = 0; i < 4; i++) {
      const grip = renderer_.xr.getControllerGrip(i);
      if (grip?.userData?.xrInputSource === src) {
        grip.userData.handedness = src.handedness;
      }
      const ctrl = renderer_.xr.getController(i);
      if (ctrl?.userData?.xrInputSource === src) {
        ctrl.userData.handedness = src.handedness;
      }
    }
  }
  for (let i = 0; i < Math.min(2, session.inputSources.length); i++) {
    ...
    const grip = renderer_.xr.getControllerGrip(i);
    const ctrl = renderer_.xr.getController(i);
    if (grip && !grip.userData?.handedness) {
      grip.userData.handedness = src.handedness;
      grip.userData.xrInputSource = src;
    }
    if (ctrl && !ctrl.userData?.handedness) {
      ctrl.userData.handedness = src.handedness;
    }
  }
}
...
const SAMPLED_SFX = {
  rifleFire: chasevrAudioUrl("freesound_community-sniper-rifle-5989.mp3"),
  healthPickup: chasevrAudioUrl("submarine-sonar-38243-once.mp3"),
  explosionImpact: chasevrAudioUrl("sound-design-elements-impact-sfx-ps-084-353199.mp3"),
  metalHits: [
    chasevrAudioUrl("metal-hit-92-200420.mp3"),
    chasevrAudioUrl("metal-hit-94-200422.mp3"),
    chasevrAudioUrl("metal-hit-95-200424.mp3"),
  ],
  ...
  droneDeath: chasevrAudioUrl("dragon-studio-car-crash-sound-effect-376874.mp3"),
  /* Arrow release / flight — same samples as VRrunner (ChaseVR/audio/). */
  bowSwooshes: [
    chasevrAudioUrl("arrow-swoosh.mp3"),
    chasevrAudioUrl("arrow-swoosh2.mp3"),
  ],
};
...
export function initBots(opts) {
  ...
  shatterRunnerGlassIfHit_ = opts.shatterRunnerGlassIfHit || null;
  ...
  canToggleArrowTypeOnX_ = opts.canToggleArrowTypeOnX || null;
...
/** Same teardown + respawn as `restartRun`, but leaves combat OFF (VRrunner pit fall). */
export function resetRunWithoutStartingCombat() {
  _resetGameCore(false);
}
...
/* ── VRragdoll sandbox: same archery as VRrunner, no combat waves ─────── */

/** @type {((prev: THREE.Vector3, dir: THREE.Vector3, segLen: number, speed: number) => { t: number, point?: THREE.Vector3 } | null) | null} */
let arrowRagdollSegmentHit_ = null;
/** @type {((center: THREE.Vector3, radius: number, push: number) => void) | null} */
let arrowRagdollExplosion_ = null;

/**
 * @param {{ segmentHit?: typeof arrowRagdollSegmentHit_, explosion?: typeof arrowRagdollExplosion_ }} cbs
 */
export function setArrowRagdollCallbacks(cbs) {
  arrowRagdollSegmentHit_ = cbs?.segmentHit || null;
  arrowRagdollExplosion_ = cbs?.explosion || null;
}

/**
 * Bow + arrows without initBots HUD / drones. Uses the same draw, flight,
 * stick, explosive, and grapple code paths as VRrunner.
 * @param {object} opts — scene, camera, cameraRig, renderer, getCollisionBoxes, getPlayerVelocity
 */
export function initArcherySandbox(opts) {
  scene_ = opts.scene;
  camera_ = opts.camera;
  cameraRig_ = opts.cameraRig;
  renderer_ = opts.renderer;
  ...
  getPlayerVelocity_ = opts.getPlayerVelocity || (() => null);
  getPlayerPosition_ = opts.getPlayerPosition || (() => cameraRig_?.position || new THREE.Vector3());
  shatterRunnerGlassIfHit_ = null;
  playerHp_ = 999;
  gameOver_ = false;
  enabled_ = false;
  battleOnBEnabled_ = false;
  editorLocomotionActive_ = false;
  initDone_ = true;
  attachHandednessTagging();
  _ensureArcherySetup();
}

/** VR input + archery integration (call once per frame in VR). */
export function updateArcherySandbox(dt) {
  if (!initDone_) return;
  pollVRInputsArchery();
  if (ARCHERY_ENABLED) updateArrows(dt);
}
...
/**
 * @param {(arrow: { prev: THREE.Vector3, pos: THREE.Vector3, vel: THREE.Vector3, speed: number }) => void} fn
 */
export function visitInFlightArrows(fn) {
  for (let i = 0; i < arrows_.length; i++) {
    const a = arrows_[i];
    if (!a?.mesh) continue;
    fn({
...
/** Vehicles / helicopters (ChaseVR chase police, player car, heli). */
