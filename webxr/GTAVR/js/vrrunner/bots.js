...
/** When set, left X only cycles arrow type if this returns true (GTAVR: foot mode). */
...
/** When false the bow stays stowed (GTAVR holster); VRrunner keeps this true by default. */
...
function _getControllerByHand(hand) {
  ...
  /* Prefer grips — GTAVR (and VRrunner) parent visible hands here; grip
...
/** VRrunner: right A swaps bow hand. GTAVR foot: holster picks hand; A cycles arrow type. */
...
function pollVRInputs() {
  ...
  if (ARCHERY_ENABLED && bowEquipped_) {
    ...
    /* Right A: swap bow hand (VRrunner) or cycle arrow type (GTAVR foot). */
