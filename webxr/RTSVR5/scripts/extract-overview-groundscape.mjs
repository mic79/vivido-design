#!/usr/bin/env node
/**
 * Build Overview dirt+rocks groundscape (~1MB textures) for skirmish props.
 * Never load the 89MB catalog at runtime.
 *
 *   node RTSVR5/scripts/extract-overview-groundscape.mjs
 */
...
const KEEP_RE = /SM_Rock|SM_Dirt/i;
...
function main() {
  ...
  const outNodes = [{ name: 'OverviewGroundscape', children: [] }];
  ...
  const out = {
    asset: { ...(json.asset || {}), generator: 'extract-overview-groundscape' },
