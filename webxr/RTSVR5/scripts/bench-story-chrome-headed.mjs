#!/usr/bin/env node
/**
 * PCVR-path Story bench = headed Google Chrome on this machine (vsync on).
 * Same method as RTSVR4/bench-chrome-headed.mjs — not headless Playwright.
 *
 *   node RTSVR5/scripts/bench-story-chrome-headed.mjs
 *
 * Env: SAMPLE_MS=4000 STORY_SEED=42
 */
...
const OUT = path.join(WEBXR, 'RTSVR5', 'bench-story-chrome-headed');
const SAMPLE_MS = Number(process.env.SAMPLE_MS || 4000);
const STORY_SEED = Number(process.env.STORY_SEED || 42) >>> 0;

const PROJECTS = [
  { id: 'RTSVR4', root: path.join(WEBXR, 'RTSVR4'), port: 8794 },
  { id: 'RTSVR5', root: path.join(WEBXR, 'RTSVR5'), port: 8795 },
  { id: 'RTSVR5Forest', root: path.join(WEBXR, 'RTSVR5Forest'), port: 8796 },
];
...
async function sampleHud(page, shotPath) {
  ...
  const snap = await page.evaluate(() =>
    window.__rtsPerf && typeof window.__rtsPerf.snapshot === 'function' ? window.__rtsPerf.snapshot() : null
  );
  return {
    samples: rows.length,
    ...
    perfFps: snap ? snap.fpsAvg ?? snap.fps : null,
    perfFrameMs: snap && snap.avgMs ? snap.avgMs.frame : null,
    perfCallsAvg: snap && snap.gpu ? snap.gpu.callsAvg : null,
    perfTrisAvg: snap && snap.gpu ? snap.gpu.trisAvg : null,
  };
}
...
async function benchProject(proj, browser) {
  ...
  try {
    ...
    result.poses.push({
      pose: 'story-look-center',
      ...(await sampleHud(page, path.join(shotDir, '02-story-look-center.png'))),
    });
    result.version = result.poses.at(-1).version;
    console.log(
      `${proj.id} look-center HUD ${result.poses.at(-1).fpsHudAvg?.toFixed(0)} fps d=${result.poses.at(-1).callsLast} tK=${result.poses.at(-1).trisK}`
    );
    ...
    result.poses.push({
      pose: 'story-look-out',
      ...(await sampleHud(page, path.join(shotDir, '03-story-look-out.png'))),
    });
    console.log(
...
async function main() {
  ...
  console.log('Launching headed Google Chrome (PCVR desktop path, vsync on)…');
  ...
  const pack = {
    ...
    method: {
      harness: 'RTSVR5/scripts/bench-story-chrome-headed.mjs',
      sameAs: 'RTSVR4/bench-chrome-headed.mjs',
      browser: 'Google Chrome channel=chrome headed=true',
      vsync: 'on (not disabled)',
      mode: 'story',
      storySeed: STORY_SEED,
      sampleMs: SAMPLE_MS,
      note: 'Headed Chrome on this PC GPU — same path used for prior PCVR desktop benches. Not Quest standalone.',
    },
    projects: results,
  };
  const jsonPath = path.join(OUT, 'bench-story-chrome-headed.json');
  fs.writeFileSync(jsonPath, JSON.stringify(pack, null, 2));

  console.log('\n=== headed Chrome Story HUD FPS (this machine, vsync on) ===');
  console.log(
    `${'project'.padEnd(14)} ${'ver'.padEnd(8)} ${'pose'.padEnd(20)} ${'fpsAvg'.padStart(7)} ${'fpsMin'.padStart(7)} ${'d'.padStart(5)} ${'tK'.padStart(6)}`
  );
  for (const p of results) {
    for (const row of p.poses) {
      console.log(
        `${p.id.padEnd(14)} ${String(row.version || '?').padEnd(8)} ${row.pose.padEnd(20)} ${String(row.fpsHudAvg != null ? row.fpsHudAvg.toFixed(0) : '-').padStart(7)} ${String(row.fpsHudMin != null ? row.fpsHudMin : '-').padStart(7)} ${String(row.callsLast).padStart(5)} ${String(row.trisK).padStart(6)}`
      );
    }
  }
  console.log(`\nWrote ${jsonPath}`);
