#!/usr/bin/env node
/**
 * Binary-strip Story kit LOD2 to natural scenery (rocks/cliffs/dirt/minerals/water).
 * Keeps quantized/compressed buffers — much smaller than a float GLTFExporter rebake.
 *
 *   node RTSVR5/scripts/extract-story-rocks.mjs
 */
...
const KEEP_RE = /SM_Rock|SM_Dirt|SM_Cliff|SM_Mineral|SM_WaterPlane/i;
...
function main() {
  ...
  for (const e of json.extensionsRequired || []) extUsed.add(e);
  for (const mat of materials) {
    if (mat.extensions) for (const e of Object.keys(mat.extensions)) extUsed.add(e);
  }
  // Drop unused extensions that need plugins we didn't copy payloads for.
  const keepExt = [...extUsed].filter((e) =>
    /KHR_mesh_quantization|KHR_materials_unlit|KHR_texture_transform/i.test(e)
  );

  const out = {
    asset: { ...(json.asset || {}), generator: 'extract-story-rocks' },
  ...
  console.log(`wrote ${path.relative(ROOT, DST)} (${mb} MB) rocks=${rockOnly.length} mats=${materials.length} images=${images.length} ext=${keepExt.join(',')||'-'}`);
}
