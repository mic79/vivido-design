...
import {
  ...
  RESOURCE_FIELD_CAPACITY, UNIT_CAP_PER_PLAYER,
  getResourceFieldPositions,
} from './config.js';
...
export function initResourceFields() {
  ...
  getResourceFieldPositions().forEach((pos, i) => {
...
export const gameSession = {
  ...
  menuOpen: true,
  /** True while Story/skirmish map generate + terrain rebuild is in progress (blocks re-clicks). */
  matchPreparing: false,
  /** Active match mode (`story`, `1v1`, …) once a match starts. */
  matchMode: null,
  /** Story layout seed for the active (or last-started) Story match. */
  storySeed: null,
  /** Compact Story layout meta for history rows. */
  storyMeta: null,
  /** Prevents double-writing Story history when gameOver is polled every frame. */
  storyHistoryRecorded: false,
  ...
  /** Nav walkability debug: blue = walkable on minimap + world plane. N key, or `?navDebug=1`. */
  navDebug: false,
...
export function resetState() {
  ...
  // Do not clear matchPreparing here — prepare overlay owns that flag until spawn finishes.
  gameSession.matchMode = null;
