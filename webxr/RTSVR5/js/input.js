...
// VR controller state (grip pan matches RTSVR4 rts-controller: rig-local deltas + rotY)
const vrLeft = {
...
/** Both grips: pinch height like RTSVR4 (world hand distance delta). */
...
/** Child [data-vr-aim-ray] uses RTSVR4-style -90° X so local -Z is forward from the grip. */
...
function tryVrUiClickFromChildRay(controllerElRaw) {
  ...
  try {
    ...
    console.warn('[RTSVR4] VR UI click handler failed', err);
  }
  ...
  try {
    ...
    console.warn('[RTSVR4] VR UI click emit failed', err2);
    return false;
  }
  tryVrControllerPulse('right', 0.44, 34);
  return true;
...
export function initInput(sceneEl) {
  ...
  window.addEventListener('keydown', e => {
    ...
    if ((e.key === 'n' || e.key === 'N') && State.gameSession.sceneContentReady && !State.gameSession.menuOpen) {
      e.preventDefault();
      State.gameSession.navDebug = !State.gameSession.navDebug;
      NavDebug.syncNavDebugOverlayFromState();
      UI.updateMinimap();
      UI.showStatus(State.gameSession.navDebug ? 'Nav map: ON (blue = walkable)' : 'Nav map: OFF');
    }
...
export function jumpCameraTo(x, z) {
  const camClamp = clampWorldToCameraNavDisk(x, z);
...
export function positionCameraForPlayer(playerId) {
  ...
  const c = clampWorldToCameraNavDisk(cameraRig.x, cameraRig.z);
  cameraRig.x = c.x;
  cameraRig.z = c.z;
...
export function updateInput(dt) {
  ...
  // VR camera controls (RTSVR4 rts-controller semantics: local hand motion under #cameraRig)
    ...
    if (rigEl && lh && rh) {
      ...
      if (bothGrips) {
        ...
        const applyGripPan = (handEl, state) => {
          ...
          const cg = clampWorldToCameraNavDisk(cameraRig.x + _worldDelta.x, cameraRig.z + _worldDelta.z);
          cameraRig.x = cg.x;
          cameraRig.z = cg.z;
          state._gripRef.copy(_handLocal);
        };
...
function performWorldCommandRay(origin, direction, pickNdc) {
  ...
  if (hitResource && !hitResource.depleted && harvesterIds.length > 0) {
    ...
    Renderer.showOrderConfirm(hitResource.x, hitResource.z, 'harvest');
    return;
  }
...
function onTouchMove(e) {
  ...
  if (e.touches.length >= 2 && touchTwin && touchTwin.idA != null && touchTwin.idB != null) {
    ...
    const c0 = clampWorldToCameraNavDisk(cameraRig.x, cameraRig.z);
    cameraRig.x = c0.x;
    cameraRig.z = c0.z;
