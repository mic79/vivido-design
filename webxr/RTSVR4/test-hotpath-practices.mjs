...
assert.match(moonSrc, /MeshLambertMaterial/);
assert.match(moonSrc, /cheapMoonLook/);
assert.match(moonSrc, /moonLivePbrRequested/);
assert.match(moonSrc, /_getDynamicShadowsEnabled/);
assert.match(bakedSrc, /nobake/);
assert.match(bakedSrc, /livepbr/);
assert.match(bakedSrc, /lambert\+glb-moon01/);
...
const uiSrc = fs.readFileSync(new URL('./js/ui.js', import.meta.url), 'utf8');
const vrMenuSrc = fs.readFileSync(new URL('./js/vr-menu-aframe.js', import.meta.url), 'utf8');
const idxSrc = fs.readFileSync(new URL('./index.html', import.meta.url), 'utf8');
assert.match(rendSrc, /export function toggleDynamicShadows/);
assert.match(rendSrc, /shadowMap\.enabled = on/);
assert.match(rendSrc, /shadow\.map\.dispose/);
assert.match(rendSrc, /if \(!_dynamicShadowsOn\)/);
assert.match(rendSrc, /_shadowGpuDirty/);
assert.match(rendSrc, /placeDirectionalLightForNavCoverage/);
assert.match(rendSrc, /_shadowGpuDirty \|\| _shadowCasterSig === null/);
assert.match(rendSrc, /obj\.receiveShadow = on/);
assert.match(rendSrc, /getObject3D\('mesh'\)/);
assert.match(rendSrc, /shadowMapWantsGpuPass/);
assert.match(rendSrc, /_pausedStaticUploaded && !shadowMapWantsGpuPass/);
assert.match(vrMenuSrc, /action === 'shadows'/);
assert.match(idxSrc, /id="vr-btn-shadows"/);
assert.match(idxSrc, /id="vr-hud-shadows-btn"/);
assert.match(idxSrc, /rts-vr-menu-btn="action: shadows"/);
assert.match(uiSrc, /syncDynamicShadowToggleUi/);
assert.match(uiSrc, /id="btn-toggle-shadows"/);
assert.match(uiSrc, /id="hud-shadows-toggle"/);
