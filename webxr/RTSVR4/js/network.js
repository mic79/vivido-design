...
/** BattleVR-style: host id is `RTSVR4-host-{N}` so joiners only pick lobby 1–4 (no pasted PeerJS id). */
...
function hostSessionId() {
  return `RTSVR4-host-${selectedLobby}`;
}
...
export function commandFailureMessage(code) {
  const M = {
    ...
    cant_sell_hq: 'The HQ cannot be sold.',
    not_sellable_unit: 'That unit cannot be sold.',
    not_near_war_factory: 'Move vehicles next to your War Factory to sell them.',
    no_sellable_vehicles: 'No selected vehicles are in sell range.',
  };
...
export async function startHosting() {
  ...
  try {
    ...
    peer.on('disconnected', () => {
      console.warn('[RTSVR4] PeerJS server disconnected (host)');
      setLobbyMenuStatus(
...
export async function joinGame() {
  ...
  const clientId = `RTSVR4-client-${Date.now().toString(36)}`;
  ...
  try {
    ...
    peer.on('disconnected', () => {
      console.warn('[RTSVR4] PeerJS server disconnected (client)');
      setLobbyMenuStatus(
...
function handleClientData(data, fromPlayerId) {
  ...
  if (data?.type === 'command') {
    ...
    try {
      ...
      console.error('[RTSVR4] Host command execution error', err);
  ...
  switch (data.type) {
    ...
    case 'snapshot': {
      try {
        ...
        console.error('[RTSVR4] applySnapshot failed', err);
      }
      break;
    }
      ...
      import('./ui.js')
        .then(async m => {
          ...
          try {
            ...
            if (typeof inp.getIsVR === 'function' && !inp.getIsVR()) {
    ...
    default:
      if (data.type) console.warn('[RTSVR4] Unknown message from host:', data.type);
      break;
  }
}
...
function executeCommand(data, actingPlayerId) {
  ...
  switch (data.action) {
    ...
    case 'sellBuilding': {
      const code = Buildings.sellBuilding(data.buildingId, actingPlayerId);
      if (code) return { ok: false, code };
      return { ok: true };
    }
    case 'sellVehicles': {
      const ids = filterUnitsOwned(actingPlayerId, data.unitIds);
      const r = Units.sellVehiclesForPlayer(ids, actingPlayerId);
...
function clearClientNavInterpCache(unit) {
  unit._iNavPath = null;
  unit._iNavCumLen = null;
  unit._iNavTotalLen = 0;
}

/**
 * MP client only: host sends positions, not paths. When the straight chord between the last
 * rendered position and the new snapshot crosses blocked nav (crater rim, buildings), linear
 * interpolation would cut through. Precompute an A* polyline on the same nav grid as the host
 * so display motion routes around obstacles between samples.
 */
function primeClientNavInterpCache(unit) {
  clearClientNavInterpCache(unit);
  if (!State.gameSession.isMultiplayer || State.gameSession.isHost) return;
  ...
  if (Pathfinding.isWorldMovementSegmentWalkable(x0, z0, x1, z1)) return;

  const path = Pathfinding.findPath(x0, z0, x1, z1);
  ...
  unit._iNavPath = path;
  unit._iNavCumLen = cum;
  unit._iNavTotalLen = total;
}

/** @returns {{ x: number, z: number } | null} */
function sampleClientNavInterpCache(unit, u01) {
  const path = unit._iNavPath;
  const cum = unit._iNavCumLen;
  const total = unit._iNavTotalLen;
  if (!path || path.length < 2 || !cum || !Number.isFinite(total) || total < 1e-4) return null;
...
function reseedClientUnitInterpolation(unit, uData) {
  ...
  unit._iT0 = now;
  unit._iDur = dur;
  primeClientNavInterpCache(unit);
}
...
export function smoothNetClientUnitPositions(dt) {
  ...
  State.units.forEach(unit => {
    ...
    const s = stp01(u);
    const navPos = sampleClientNavInterpCache(unit, s);
    if (navPos) {
      unit.x = navPos.x;
      unit.z = navPos.z;
    } else {
...
function applySnapshot(snapshot) {
  ...
  if (
    ...
    console.warn('[RTSVR4] applySnapshot: invalid snapshot shape (expected players/units/buildings arrays)');
    return;
  }
  ...
  if (lastClientSnapWallMs > 0) {
     ...
     */
    if (gap > 1000) {
      console.warn(`[RTSVR4 net] snapshot gap ${Math.round(gap)} ms (seq ${snapSeq})`);
    }
  }
  ...
  if (isNetClient) {
    ...
    if (wall - clientSnapLastLogMs > 5000) {
      ...
      console.log(
        `[RTSVR4 net] client applied ${clientSnapAppliedCount} snapshots (last seq=${snapSeq}, ` +
  ...
  snapshot.buildings.forEach(bData => {
    // Host normally omits dead buildings; if an hp<=0 entry appears, drop it and refresh nav.
    if (bData.hp != null && bData.hp <= 0) {
      if (State.buildings.has(bData.id)) {
        State.removeBuilding(bData.id);
        navDirty = true;
      }
      return;
    }
  ...
  // Clients do not simulate movement (host-authoritative), but they need the same walkability
  // grid for nav debug and for MP display interpolation along A* between snapshot endpoints.
