...
function unitCanDefendThreatLocally(u, threatUnit) {
  ...
  const home = unitHomeDefendAnchor(u);
  // Keep pads inside one base (~30–45 m). Neighbor Story bases are typically ≥95 m apart.
  const homeR = u.botRole === 'garrison' ? BOT_DEFEND_RADIUS + 8 : BOT_DEFEND_RADIUS + 14;
  if (home) {
    return Pathfinding.getDistance(home.x, home.z, threatUnit.x, threatUnit.z) <= homeR;
  }
  return Pathfinding.getDistance(u.x, u.z, threatUnit.x, threatUnit.z) <= BOT_DEFEND_RADIUS + 8;
}
...
function handleDefense(player, combatUnits, threats) {
  ...
  // Resolve live threat units (drop stale ids)
  const liveThreats = [];
  for (let i = 0; i < threats.length; i++) {
    const tu = State.units.get(threats[i].id);
    if (tu && tu.hp > 0) liveThreats.push(tu);
  }
  if (liveThreats.length === 0) return;

  // Sort by distance to nearest of our buildings (not only primary HQ) so remote bases defend first.
  ...
  for (let ti = 0; ti < maxThreats; ti++) {
    ...
    if (unitIsLongRangeThreat(threatUnit)) {
      const capable = available.filter(u => canCounterLongRange(u));
      if (capable.length > 0) available = capable;
    }

    const cap = isMinorThreat ? 2 : Math.min(6, available.length);
    available = available.slice(0, cap);
  ...
  if (barracks && player.credits >= 175 && liveThreats.length > 2) {
