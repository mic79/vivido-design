/**
 * preview2 — closer to Epic GLTFWebViewer / UE Lit fidelity.
 * Stable best remains epicLightmaps.js + preview.html (do not regress that).
 *
 * Differences vs v1:
 *  - Raw Epic HQ decode (no invent LM tonemap) — matches NodeLightmap.addLightMap()
 *  - directionality hardcoded 0.6 like Epic viewer
 *  - Max anisotropy on albedo / MR / normal / emissive maps
 *  - Metals: LM * PI into Standard + scene-captured IBL (set by preview2)
...
// Epic GLTFWebViewer does NOT tonemap LM before addLightMap — only soft HDR clamp for display
vec3 epicToneMapLM(vec3 lm, float contrast, float whitePoint) {
  lm = max(lm, vec3(0.0));
  // Optional mild exposure from whitePoint slider (1.0 = Epic-raw)
  lm *= max(whitePoint, 0.05);
  // Soft shoulder so HDR peaks don't blow ACES; no shadow crush
  lm = lm / (1.0 + lm * 0.35);
  if ( contrast != 1.0 ) {
    lm = pow( max(lm, vec3(1e-4)), vec3(contrast) );
  }
  return lm;
}
`;
...
/** Sharper glancing-angle detail (closer to UE texture filtering). */
function boostAnisotropy(tex, maxAniso) {
  if (!tex || !maxAniso) return tex;
  tex.anisotropy = Math.max(tex.anisotropy || 1, maxAniso);
  tex.needsUpdate = true;
  return tex;
}

function boostMaterialMaps(mat, maxAniso) {
  if (!maxAniso) return;
  for (const key of ['map', 'metalnessMap', 'roughnessMap', 'normalMap', 'aoMap', 'emissiveMap', 'alphaMap']) {
    if (mat[key]) boostAnisotropy(mat[key], maxAniso);
  }
}

function classifyMaterial(src) {
  const n = (src.name || '').toLowerCase();
  // LED tickers / monitors / pure emissives — Standard so emissiveMap UVs + HDR glow survive
  if (/master_emissive|emissive_simple|monitor_01_monitor|^emissive/.test(n)) {
    return 'emissive';
  }
  ...
  // UE hallway floors/tiles/wall panels are metallic (def metalness=1 + MR scratch maps).
  // Forcing MeshBasic here was wiping roughness scratches, text, and metal response.
  if (
    src.metalnessMap &&
    /floor|tile|fond_wall|office_|server_|fusebox/.test(n)
  ) {
    return 'metal';
  }
  // Explicit dielectrics (painted plastics, foliage, posters — not metallic architectural panels)
  if (
    /bench|divider|plant|robot|shelf|worldgrid|dyson|placeholder|glass|master_alpha|alpha_simple|pvc|poster|kitbash|plastic/.test(
      n,
    )
  ) {
    return 'dielectric';
...
function makeStandardLitMaterial(src, params, isMetal, opts = {}) {
  ...
  if (src.emissiveMap) {
    ...
    // Preserve texCoord (monitors use UV1)
    if (src.emissiveMap.channel != null) mat.emissiveMap.channel = src.emissiveMap.channel;
  }
...
function makeLitMaterial(src, params) {
  ...
  // LED / monitors: Standard so emissiveMap texCoord + HDR factors work (not MeshBasic UV0 hack)
  if (kind === 'emissive') return makeStandardLitMaterial(src, params, false);
...
export async function applyEpicLightmaps(gltf, opts = {}) {
  ...
  const whitePoint = opts.whitePoint ?? 1.0;
  ...
  for (let nodeIndex = 0; nodeIndex < json.nodes.length; nodeIndex++) {
    ...
    for (const mesh of targets) {
      ...
      const outMats = srcMats.map((src) => {
        ...
        // Per-instance LM scale/bias must stay in the cache key — sharing materials
        // across instances and mutating uniforms in onBeforeRender causes visible flicker.
        const key = [
          src.uuid,
          kind,
          lm.texture.index,
...
/** Play fan / prop clips; skip camera path animations. */
export function playGltfAnimations(gltf, root) {
...
/** UE Master_Emissive_Panner UV scroll (not exported as glTF anim). */
export function updateEmissivePanners(root, dt, speed = 0.14) {
  if (!root || !dt) return;
  root.traverse((obj) => {
    if (!obj.isMesh && !obj.isSkinnedMesh) return;
