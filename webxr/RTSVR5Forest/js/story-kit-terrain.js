...
export const FOREST_TREES_QUEST_GLB = 'assets/terrain/forest-trees-quest.glb';
/**
...
export const STORY_ROCKS_UE_GLB = 'assets/terrain/scifi-rts-rocks.glb';
    ...
    if (obj.material) {
      ...
      for (const mat of mats) {
        ...
        if (kind === 'rocks') {
          ...
          // Forest billboards: MASK cutouts (BLEND → overdraw + sort thrash on Quest).
          if (opts.url && /forest-trees|forest-road-night/i.test(opts.url)) {
            const foliage =
              mat.transparent ||
              (mat.alphaTest || 0) > 0 ||
...
export async function tryLoadRocksKit() {
  ...
  let used = urls[0];
  if (!rocksBufCache || !urls.includes(rocksBufUrl)) {
    rocksBufCache = null;
    for (const url of urls) {
      let res;
      try {
      ...
      if (!res.ok) continue;
      const next = await res.arrayBuffer();
      const minBytes = /(?:forest-trees-quest|forest-road-night-quest)\.glb/i.test(url)
        ? MIN_QUEST_KIT_BYTES
        : /forest-road-night\.glb/i.test(url)
          ? MIN_QUEST_KIT_BYTES
          : MIN_STORY_ROCKS_BYTES;
      if (next.byteLength < minBytes) continue;
      try {
        parseGlbJson(next);
      } catch (err) {
        console.warn('[RTSVR5] skip rocks kit: bad GLB', url, err);
        continue;
      }
      rocksBufCache = next;
      rocksBufUrl = url;
  ...
  if (!rocksBufCache) {
    console.warn('[RTSVR5] skip rocks kit: missing', urls);
    return null;
  }

  const gltf = await parseKitBuf(rocksBufCache);
  const isForestScene = /forest-road-night/i.test(used);
  return assembleKitWrap(gltf, {
    kind: 'rocks',
    skipIndoor: true,
    clipRadius: isForestScene ? 0 : 420,
    noPlate: isForestScene,
    skipDistanceLod: isForestScene,
    bytes: rocksBufCache.byteLength,
    url: used,
    quest: /(?:scifi-rts-quest|forest-.*-quest)\.glb/i.test(used),
  });
}
...
export async function tryLoadQuestRocksProps() {
  ...
  const urls = (() => {
    try {
      const q = location.search || '';
      // Explicit UE rocks / quest encode A/B.
      if (/(?:^|[?&#])uerocks=1(?:&|$)/i.test(q)) {
        return wantQuestAssets()
  ...
  for (const url of urls) {
    ...
    const next = await res.arrayBuffer();
    const minBytes =
      url === STORY_KIT_QUEST_GLB || /forest-trees-quest\.glb/i.test(url)
        ? MIN_QUEST_KIT_BYTES
        : MIN_STORY_ROCKS_BYTES;
    if (next.byteLength < minBytes) continue;
    try {
