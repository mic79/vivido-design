...
import { tryLoadStoryKit, tryLoadRocksKit, tryLoadOverviewKit, tryLoadOverviewGroundscape, tryLoadQuestRocksProps, rasterizeKitHeights, setupStoryKitDistanceLod, setupForestTileDistanceLod, seatForestInstancesOnMoon, resetKitLodState, applyLeanRocksHideBuildings, hasStoryKitLodFor } from './story-kit-terrain.js';
...
export async function ensureFocusCullPipeline(groundEl) {
  ...
  const lodRoot =
    props ||
    (mesh && (mesh.userData?.rtsKitKind || mesh.userData?.rtsStoryKit || mesh.userData?.rtsForestProps)
      ? mesh
      : null);
  if (!lodRoot) return;
  ...
  if (lodRoot.userData?.rtsForestProps) {
    setupForestTileDistanceLod(lodRoot, THREE);
  }
}
