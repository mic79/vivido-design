...
/** BattleVR-style: host id is `RTSVR5-host-{N}` so joiners only pick lobby 1–4 (no pasted PeerJS id). */
...
function hostSessionId() {
  return `RTSVR5-host-${selectedLobby}`;
}
...
export async function joinGame() {
  ...
  const clientId = `RTSVR5-client-${Date.now().toString(36)}`;
