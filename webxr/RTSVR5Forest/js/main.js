...
function initializeGame(sceneEl) {
  console.log('🎮 RTSVR5 Initializing...');
...
async function prepareMapForMode(mode, sceneEl, prevProfile) {
  ...
  const groundMesh = groundEl && groundEl.getObject3D && groundEl.getObject3D('mesh');
  const groundIsKit = !!(groundMesh && groundMesh.userData && groundMesh.userData.rtsKitKind);
  const groundIsMoon = !!(groundMesh && groundMesh.userData && groundMesh.userData.rtsSkirmishBake);
  // Kit style with crater moon still resident (lobby boot race / stale cache) → force remount.
  const needKitRemount = MAP_TERRAIN_STYLE === 'kit' && !groundIsKit;
  const needMoonRemount = MAP_TERRAIN_STYLE !== 'kit' && (groundIsKit || !groundIsMoon) && !wantStory;
  ...
  if (prevProfile !== nextProfile || wantStory || needKitRemount || needMoonRemount) {
...
async function onStartGame(mode) {
  ...
  try {
    try {
      ...
      console.error('[RTSVR5] Map prepare failed', err);
