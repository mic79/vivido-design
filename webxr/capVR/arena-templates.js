 ...
 */
const ArenaManager = {
  STORAGE_KEY_PREFIX: 'capvr_arena_',
  ...
  // HTML embeds classic One; CapVR swaps to Three after snapshot (see init).
  currentArenaName: 'One',
  _appliedArenaName: null, // set only after loadLayout actually applies geometry
  currentArenaSource: 'official', // 'official' or 'personal'
  officialArenas: [
    { id: 'zero', name: 'Zero', description: 'Empty arena for testing', file: 'zero.json' },
    { id: 'one', name: 'One', description: 'The classic CapVR arena', file: 'one.json' },
  ...
  init: async function() {
    ...
    const startDefault = () => {
      this.captureDefaultSnapshot().then(() => this.loadCapVRDefaultArena());
    };
    if (scene.hasLoaded) {
      startDefault();
    } else {
      scene.addEventListener('loaded', startDefault, { once: true });
    }
  ...
  /** CapVR boots into arena Three (not the HTML-embedded One). */
  loadCapVRDefaultArena: async function() {
    if (this._appliedArenaName && this._appliedArenaName !== 'One') return;
    try {
      console.log('🗺️ Loading CapVR default arena: Three');
      await this.loadOfficialArena(this.DEFAULT_OFFICIAL_ID || 'three');
      this.updateMenuDisplay();
    } catch (e) {
      console.error('❌ Failed to load default arena Three:', e);
    }
  },
  ...
  captureDefaultSnapshot: function() {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        this.defaultArenaSnapshot.metadata.description = 'The classic CapVR arena';
        resolve(this.defaultArenaSnapshot);
      }, 1000);
    });
  },
  ...
  getCurrentLayout: function() {
    ...
    const objects = [];
    const isFixture = (entity) => {
      if (window.CapVRArenaFixtures?.isArenaFixture?.(entity)) return true;
      if (entity.classList?.contains('arena-fixture')) return true;
      if (entity.closest?.('#arena-fixtures')) return true;
      if (entity.hasAttribute?.('data-arena-fixture')) return true;
      return false;
    };
    ...
    arenaObjects.forEach(entity => {
      ...
      if (!hasGoalRing && !isChildOfGoal && !isFixture(entity)) {
        try {
          const data = serializeArenaObject(entity);
    ...
    const fixtures = window.CapVRArenaFixtures?.captureFixtures?.() || null;
  ...
  loadLayout: async function(layoutData, arenaName) {
    // Skip only if this arena was actually applied (not just name-preset)
    if (this._appliedArenaName === arenaName) {
      return;
    }
    ...
    this._appliedArenaName = arenaName;

    // CapVR: goals / spawn pyramids are stored separately from prop objects
    if (layoutData.fixtures && window.CapVRArenaFixtures?.applyFixtures) {
      try {
        window.CapVRArenaFixtures.applyFixtures(layoutData.fixtures);
      } catch (e) {
        console.warn('[CapVR] applyFixtures failed', e);
      }
    } else if (window.CapVRArenaFixtures?.syncHomesFromGoals) {
      try {
        window.CapVRArenaFixtures.syncHomesFromGoals({ moveHomeFlag: true });
      } catch (e) { /* */ }
    }
  ...
  clearArena: function() {
    ...
    arenaObjects.forEach(entity => {
      ...
      if (!hasGoalRing && !isChildOfGoal && !isFixture(entity) && entity.parentNode) {
  ...
  getDefaultArenaLayout: function() {
    ...
    currentLayout.metadata.description = 'The classic CapVR arena';
