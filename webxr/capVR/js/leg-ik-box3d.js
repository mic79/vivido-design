...
(function () {
  ...
  AFRAME.registerComponent('leg-ik-world', {
    ...
    /** CapVR/BoltVR: #player is world root, #rig is a child. Never write world coords into #rig. */
    _isCapVRHierarchy: function () {
      const player = document.getElementById('player');
      const rig = document.getElementById('rig');
      return !!(player && rig && player.contains(rig) && player.components && player.components['zerog-player']);
    },

    _capvrFollowBoltPlayer: function () {
      if (!this.physics) return;
      // BoltVR thrusters move #rig (child of #player) — use rig WORLD pose.
      const rig = document.getElementById('rig');
      const player = document.getElementById('player');
      ...
      if (this._playerRootPos) {
        this._playerRootPos.set(this._capvrTmp.x, feetY, this._capvrTmp.z);
      }
      if (this.scene?.legIkWorld) {
    ...
    _setVRRigFeetUnderHead: function () {
      if (this._isCapVRHierarchy()) return; // BoltVR zerog-player owns #player/#rig
      const rig = document.getElementById('rig');
      const camera = document.getElementById('camera');
    ...
    _recenterPlayer: function (reason) {
      ...
      if (this._isCapVRHierarchy()) {
        if (reason !== 'origin-jump') this._applyStandingEyeHeight(reason);
        this._capvrFollowBoltPlayer();
        this._lastRecenterMs = performance.now();
        this.scene.emit('vr-recenter', { reason: reason || 'unknown' });
        console.log('[Leg IK World] CapVR recenter (BoltVR pose preserved):', reason);
        return;
      }
    ...
    _initBox3D: async function () {
      try {
        ...
        // CapVR Echo Arena floor ≈ y=-10; body-rigged demo ground is y=0 mid-chamber.
        const capvr = this._isCapVRHierarchy();
        const defaultGroundY = capvr ? -10 : 0;
        this.terrain = new Box3DTerrainQuery({ queries: this.queries, groundY: defaultGroundY });

        await this._loadReferenceObstacles();

        const rig = document.getElementById('rig');
        const player = document.getElementById('player');
        let startX = 0, startY = defaultGroundY, startZ = 0;
        if (capvr && (rig || player)?.object3D) {
          const wp = new THREE.Vector3();
          (rig || player).object3D.getWorldPosition(wp);
          startX = wp.x;
          startY = wp.y - 0.95;
          startZ = wp.z;
        } else if (rig) {
          startX = rig.object3D.position.x;
          startZ = rig.object3D.position.z;
          startY = this.terrain.getHeightAtWorld(startX, startZ);
        }
        this.physics.initPlayerAt(startX, startY, startZ);
        this.playerBody = this.physics;
        // Only body-rigged (rig = world root) may bake physics Y into #rig.
        if (rig && !capvr) rig.object3D.position.set(startX, startY, startZ);
    ...
    _loadReferenceObstacles: async function () {
      // CapVR uses BoltVR arenas — do not inject body-rigged Problem ramp/stairs.
      if (this._isCapVRHierarchy()) {
        console.log('[Leg IK World] CapVR: skipped reference obstacles');
        return;
      }
    ...
    applyGrabPullMovementDelta: function (worldDelta, dt, hints) {
      ...
      // CapVR: BoltVR zerog-player owns wall-grab locomotion. mixamo-body grab-pull
      // must NOT bake world capsule coords into child #rig (that teleports you out of the arena).
      if (this._isCapVRHierarchy()) {
        this.grabPullLocomotionActive = false;
        this._grabPullRising = false;
        return;
    ...
    _holdPlayerForDotEffect: function () {
      // CapVR: body-dot dissolve is a body-rigged4 lab demo. Writing world coords into
      // child #rig ejects the headset from the Mixamo body — never run it here.
      if (this._isCapVRHierarchy?.() || document.querySelector('[zerog-player]')) {
        const dotFx = this.scene?._bodyDotEffect;
        if (dotFx) {
          dotFx.frozen = false;
          dotFx.active = false;
          dotFx.frozenPos = null;
        }
        return false;
      }
      const dotFx = this.scene._bodyDotEffect;
    ...
    _updatePlayerPhysics: function (deltaMs) {
      ...
      // CapVR: BoltVR zerog-player moves #player/#rig. Box3D capsule only follows.
      if (this._isCapVRHierarchy()) {
        this._capvrFollowBoltPlayer();
    ...
    _resolvePlayerVsGrabDummy: function (horizontalOnly) {
      ...
      for (let iter = 0; iter < 3; iter++) {
        ...
        // CapVR: #rig is a child of #player — never bake world coords into it.
        if (this._isCapVRHierarchy?.()) {
          // Nudge #player instead so BoltVR thrusters stay consistent
          const player = document.getElementById('player');
          if (player) {
            const curP = player.object3D.position;
            const dx = nx - cur.x;
            const dy = ny - cur.y;
            const dz = nz - cur.z;
            player.object3D.position.set(curP.x + dx, curP.y + dy, curP.z + dz);
          }
        } else {
          const rig = document.getElementById('rig');
