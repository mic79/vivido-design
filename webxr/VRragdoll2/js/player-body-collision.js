 ...
 * @param {{ climbing?: boolean, platform?: { topY: number, minX: number, maxX: number, minZ: number, maxZ: number } }} [opts]
 */
    ...
    for (let i = 0; i < probes.length; i++) {
      ...
      if (plat) {
        if (
          ...
          const depth = plat.topY + s.r - s.y;
          if (depth >= MIN_PUSH_DEPTH) {
            addLargestCorrection(_best, { x: 0, y: depth, z: 0 });
          }
        }
      }
    }
