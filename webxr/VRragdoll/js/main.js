...
hud.innerHTML = [
  ...
  'VR: <b>squeeze</b> bar/pipe to climb — <b>throw</b> by releasing with hand speed · NPC: squeeze · <b>Y</b> reset',
...
function getPlayerSupportY(rigPos, xrCamera = null) {
  ...
  if (xrCamera) {
    xrCamera.getWorldPosition(tmpV);
    if (tmpV.y >= PLATFORM_TOP_Y - 0.45 && isOverPlatformXZ(tmpV)) {
...
function updateXRInteraction(dt) {
  ...
  for (const side of ['left', 'right']) {
    ...
    grip.getWorldPosition(tmpV);
    updateHandVel(side, tmpV, dt);
    ...
    if (squeeze && !was) {
      if (grabbables.tryGrab(tmpV, side, cameraRig, grip)) {
      ...
      } else if (ragdoll.tryGrab(tmpV, side)) {
...
renderer.setAnimationLoop(() => {
  ...
  if (ragdoll?.mixer) ragdoll.mixer.update(dt);
  if (ragdoll?.mode === 'animated') ragdoll.stepKinematicBodies();

  world.timestep = dt;
  world.step();

  if (ragdoll?.mode === 'ragdoll') ragdoll.syncBonesFromPhysics();
