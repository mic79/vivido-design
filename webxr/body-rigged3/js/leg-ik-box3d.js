/**
 * Leg IK for A-Frame Mixamo bodies â€” Box3D physics (body-rigged3)
 */
(function () {
  'use strict';
  ...
  AFRAME.registerComponent('leg-ik-world', {
    ...
    toggleRagdoll: function () {
      ...
      // Carry the player's current momentum into the ragdoll so it launches/tumbles
      // instead of dropping straight down from rest.
      if (window.Box3DRagdoll.setHumanVelocity) {
        const speed = this.scene.legIkWorld?.playerSpeed || 0;
        const dir = this._playerMoveDir || { x: 0, z: 0 };
        const vy = (this.physics.playerVelY ?? this.playerVelY) || 0;
        window.Box3DRagdoll.setHumanVelocity(
          this.b3,
          human,
          dir.x * speed,
          Math.max(vy, -6),
