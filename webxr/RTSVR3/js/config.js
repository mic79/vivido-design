...
/**
 * Base radius before `MAP_CAMERA_NAV_AREA_SCALE`: inner inset from `MAP_PLAYABLE_RADIUS` (terrain / UV rim).
 * Actual gameplay clamp uses `MAP_UNIT_NAV_RADIUS`.
 */
...
/** Effective playable clamp radius (m) — same for camera and units/buildings. */
export const MAP_UNIT_NAV_RADIUS =
  MAP_UNIT_PLAYABLE_RADIUS * Math.sqrt(MAP_CAMERA_NAV_AREA_SCALE);

/**
 * Camera rig / flat pan clamp: this far beyond `MAP_UNIT_NAV_RADIUS` so the view can peek past
 * the navigable rim (units/buildings still use `MAP_UNIT_NAV_RADIUS` via `clampWorldToPlayableDisk`).
 */
export const MAP_CAMERA_NAV_EXTENSION = 0.1;
export const MAP_CAMERA_PAN_RADIUS = MAP_UNIT_NAV_RADIUS * (1 + MAP_CAMERA_NAV_EXTENSION);

/**
 * XZ span of the nav debug plane / minimap world square — must match `pathfinding.js`
 * (`CELL = 2`, `NAV_GRID_HALF = MAP_UNIT_NAV_RADIUS`). Fog grid uses the same bounds.
 */
export const MAP_NAV_PLANE_CELL = 2;
export const MAP_NAV_PLANE_COLS = Math.ceil((2 * MAP_UNIT_NAV_RADIUS) / MAP_NAV_PLANE_CELL);
export const MAP_NAV_PLANE_SPAN_M = MAP_NAV_PLANE_COLS * MAP_NAV_PLANE_CELL;
export const MAP_NAV_PLANE_HALF_M = MAP_NAV_PLANE_SPAN_M * 0.5;
...
// --- Fog of War ---
export const FOG_GRID_SIZE = 20;
export const FOG_CELL_SIZE = MAP_NAV_PLANE_SPAN_M / FOG_GRID_SIZE;
