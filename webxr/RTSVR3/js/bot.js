...
function handleDefense(player, combatUnits, threats) {
  ...
  if (!isMinorThreat && threats.length >= 3 && available.length >= 9) {
    ...
    commandBotEngageEnemyUnits(a.map(u => u.id), threats[0]);
    commandBotEngageEnemyUnits(b.map(u => u.id), threats[1]);
    commandBotEngageEnemyUnits(c.map(u => u.id), threats[2]);
  } else if (!isMinorThreat && threats.length >= 2 && available.length >= 4) {
    const half = Math.max(2, Math.floor(available.length / 2));
    commandBotEngageEnemyUnits(available.slice(0, half).map(u => u.id), threats[0]);
    commandBotEngageEnemyUnits(available.slice(half).map(u => u.id), threats[1]);
  } else {
    commandBotEngageEnemyUnits(available.map(u => u.id), primaryThreat);
  }
