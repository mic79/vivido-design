...
(function () {
   ...
   */
  const BAKED_SCENE_OFFSET_RIGHT_M = 22;
  const BAKED_SCENE_OFFSET = { x: BAKED_SCENE_OFFSET_RIGHT_M, y: 2, z: 0 };
  /**
   * Dynamic shadow sun — matched to Godot DirectionalLight3D "Sun":
   *   Rotation (deg) X=-33.4 Y=27.1 Z=0 → light travels along local −Z.
   *   (GLB export is unlit; no KHR_lights_punctual — direction must be copied from Godot.)
   * Override live: ?sunAz=27.1&sunEl=33.4
   * az: ° from +Z toward +X; el: ° above horizon.
   */
  const BAKED_SCENE_SUN_AZIMUTH_DEG = 27.1;
  const BAKED_SCENE_SUN_ELEVATION_DEG = 33.4;

  function obstacleGlbUrl(file) {
    return file + '?v=' + OBSTACLE_GLB_VERSION;
  }

  function useDemoPlayground() {
    try {
  ...
  function bakedSceneSunDirection() {
    let az = BAKED_SCENE_SUN_AZIMUTH_DEG;
    let el = BAKED_SCENE_SUN_ELEVATION_DEG;
    try {
      const q = new URLSearchParams(window.location.search);
      if (q.has('sunAz')) az = parseFloat(q.get('sunAz'));
      if (q.has('sunEl')) el = parseFloat(q.get('sunEl'));
    } catch (_) {}
    if (!Number.isFinite(az)) az = BAKED_SCENE_SUN_AZIMUTH_DEG;
    if (!Number.isFinite(el)) el = BAKED_SCENE_SUN_ELEVATION_DEG;
    el = THREE.MathUtils.clamp(el, 5, 89);
    const azR = THREE.MathUtils.degToRad(az);
    const elR = THREE.MathUtils.degToRad(el);
    const cosEl = Math.cos(elR);
    // Y-up: az 0 = +Z, 90 = +X (right when looking down −Z).
    return {
  ...
  class Box3DTerrainQuery {
    ...
    /**
     * @param {number} [fromY] — ray origin Y. Prefer feet/capsule + ~1 m so roofs
     *   above the player are not treated as the walkable surface.
     */
    getHeightAtWorld(x, z, fromY) {
      if (!this.queries) return this.groundY;
      const originY = fromY != null && Number.isFinite(fromY) ? fromY : this.rayOriginY;
      const maxDist = Math.max(5, originY - this.groundY + 5);
      const hit = this.queries.castRayDown(x, originY, z, maxDist, this.excludeShapeIds);
  ...
  class LegIK {
    ...
    _measureFootClearanceWeight(side) {
      ...
      let groundY = this.terrain.getHeightAtWorld(footWorld.x, footWorld.z, footWorld.y + 0.35);
      if (this.queries) {
    ...
    _sampleFootRaycast(side, foot) {
      ...
      let groundY = this.terrain.getHeightAtWorld(originX, originZ, originY);
    ...
    _sampleNormal(x, z) {
      const eps = 0.25;
      const fromY = (this._rootPosition?.y ?? 0) + (this.raycastHeight || 1.2);
      const hL = this.terrain.getHeightAtWorld(x - eps, z, fromY);
      const hR = this.terrain.getHeightAtWorld(x + eps, z, fromY);
      const hD = this.terrain.getHeightAtWorld(x, z - eps, fromY);
      const hU = this.terrain.getHeightAtWorld(x, z + eps, fromY);
  ...
  AFRAME.registerComponent('leg-ik-world', {
    ...
    _setVRRigFeetUnderHead: function () {
      ...
      const groundY = this.terrain.getHeightAtWorld(headWorld.x, headWorld.z, headWorld.y);
    ...
    _initBox3D: async function () {
      try {
        ...
        this.terrain = new Box3DTerrainQuery({
    ...
    /** Godot/Blender-baked unlit GLB environment (KHR_materials_unlit). */
    _loadBakedEnvironment: async function (file) {
      await this._waitForGltfLoader();
      if (!this.physics) return;
      const url = obstacleGlbUrl(file);
      ...
      await new Promise((resolve) => {
        loader.load(
          ...
          (gltf) => {
            ...
            model.name = 'zdm2-baked-environment';
            model.traverse((child) => {
              if (!child.isMesh) return;
              // Keep MeshBasic (unlit bake). Do not promote to lit materials.
              if (child.material) {
              ...
              // Bake stays MeshBasic (self-lit). It must still *cast* into the shadow
              // map so walls/floors occlude character shadows (otherwise shadows punch
              // through geometry). Receiving is handled by the ShadowMaterial overlay.
            ...
            // AABB min is often a stray overhang — seat the densest low band on Y=0.
            const groundY = this._estimateBakedGroundY(model);
            model.position.y -= groundY;
            model.updateWorldMatrix(true, true);

            // Choose the start before applying the requested scene offset. Otherwise
            // the spawn follows the GLB and the movement is visually cancelled.
            const unshiftedBox = new THREE.Box3().setFromObject(model);
            const spawn = this._pickBakedSpawn(model, unshiftedBox);

            // Player-start right = +X (camera looks down −Z); Y = world up.
            model.position.x += BAKED_SCENE_OFFSET.x;
            model.position.y += BAKED_SCENE_OFFSET.y;
            model.position.z += BAKED_SCENE_OFFSET.z;
            model.updateWorldMatrix(true, true);
            ...
            console.log(
              '[Leg IK World] Loaded baked scene:',
              file,
              'groundY',
              groundY.toFixed(2),
              'offset',
              ...
              '(trimesh only — no fake floor)'
            );
            resolve();
          },
          undefined,
          (err) => {
            console.warn('[Leg IK World] Failed to load baked scene', file, err);
            resolve();
          }
        );
      });
    ...
    /**
     * Unlit bakes cannot receive shadows, so overlay the same geometry with
     * ShadowMaterial. The bake itself casts into the shadow map (occlusion);
     * catchers are coplanar so bias/polygonOffset must suppress self-shadow.
     */
    _addBakedShadowCatcher: function (model) {
      const meshes = [];
      model.traverse((child) => {
        if (child.isMesh && child.geometry && !child.userData.isShadowCatcher) {
          meshes.push(child);
        }
      });
      ...
      for (const src of meshes) {
        ...
        const catcher = new THREE.Mesh(src.geometry, shadowMat);
        catcher.name = (src.name || 'baked') + '-shadow-catcher';
        catcher.position.copy(src.position);
        catcher.quaternion.copy(src.quaternion);
        catcher.scale.copy(src.scale);
        catcher.castShadow = false;
        catcher.receiveShadow = true;
        catcher.renderOrder = 1;
        catcher.userData.isShadowCatcher = true;
        // Pull catcher a hair toward the camera along its normal isn't reliable for
        // mixed floors/walls — rely on light shadow bias + polygonOffset instead.
        src.parent.add(catcher);
      }

      this._bakedShadowMaterial = shadowMat;
      return shadowMat;
    },

    /**
     * Aim the realtime sun at the bake bounds. Direction is authored to match the
     * bake (see BAKED_SCENE_SUN_*), not recovered from the GLB — there are no lights
     * in zdm2_baked.glb.
     */
    _fitSunShadowToBox: function (box) {
      const sun = document.getElementById('sun-light');
      const light = sun?.getObject3D?.('light');
      if (!light?.shadow || !box || box.isEmpty()) return;
      ...
      // Prefer aiming near the player if we already placed the rig.
      const rig = document.getElementById('rig');
      const aim = new THREE.Vector3();
      if (rig) {
        aim.set(rig.object3D.position.x, box.min.y + 1.0, rig.object3D.position.z);
      } else {
        aim.set(center.x, box.min.y + size.y * 0.12, center.z);
      }

      const half = Math.min(55, Math.max(28, Math.max(size.x, size.z) * 0.35 + 8));
      const dist = Math.max(80, half * 2.4);
      const { dir, azimuthDeg, elevationDeg } = bakedSceneSunDirection();
      sun.object3D.position.copy(aim).addScaledVector(dir, dist);

      // Target MUST stay in world/scene space — parenting it under the sun makes
      // position.set(worldX,…) a local offset and kills the light direction (no shadows).
      const sceneRoot = this.el.object3D;
      if (light.target.parent !== sceneRoot) {
      ...
      cam.top = half;
      cam.bottom = -half;
      cam.near = Math.max(1, dist - half * 1.5);
      cam.far = dist + half * 1.5 + size.y;
      cam.updateProjectionMatrix();

      // Mild bias: enough to limit bake↔catcher acne, not so high that character
      // contact shadows disappear (especially at ~33° elevation).
      light.shadow.bias = -0.0002;
      light.shadow.normalBias = 0.025;
      light.castShadow = true;
      if (typeof light.intensity === 'number' && light.intensity < 0.45) {
        light.intensity = 0.55;
      }

      if (light.shadow.mapSize.x < 4096) {
      ...
      console.log(
        '[Leg IK World] Bake shadow sun az/el',
        azimuthDeg.toFixed(1) + '° / ' + elevationDeg.toFixed(1) + '°',
        'aim',
        aim.x.toFixed(1),
        aim.y.toFixed(1),
        aim.z.toFixed(1)
    ...
    /** Mode of 0.5 m Y buckets in the lower half of the mesh (walkable ground). */
    _estimateBakedGroundY: function (model) {
      const ys = [];
      ...
      ys.sort((a, b) => a - b);
      const median = ys[Math.floor(ys.length * 0.5)];
      ...
      let bestY = ys[Math.floor(ys.length * 0.05)];
      let bestC = 0;
      for (const [y, c] of buckets) {
        if (c > bestC) {
          bestC = c;
          bestY = y;
        }
    ...
    /**
     * Prefer an open XZ cell with a strong near-floor band and little overhead —
     * so the y=30 ground ray used at player init hits walkable ground, not a roof.
     */
    _pickBakedSpawn: function (model, worldBox) {
      const cell = 2;
      const cells = new Map();
      const v = new THREE.Vector3();
      model.traverse((child) => {
        ...
        for (let i = 0; i < pos.count; i += step) {
          v.fromBufferAttribute(pos, i).applyMatrix4(child.matrixWorld);
          const kx = Math.floor(v.x / cell);
          const kz = Math.floor(v.z / cell);
          const key = kx + ',' + kz;
          let c = cells.get(key);
          if (!c) {
            c = { low: 0, high: 0, sx: 0, sz: 0, n: 0, minY: Infinity };
            cells.set(key, c);
          }
          c.n += 1;
          c.sx += v.x;
          c.sz += v.z;
          c.minY = Math.min(c.minY, v.y);
          if (v.y >= -0.35 && v.y <= 2.2) c.low += 1;
          if (v.y >= 6) c.high += 1;
        }
      });
      ...
      for (const c of cells.values()) {
        if (c.low < 12 || c.minY > 1.25) continue;
        const open = c.low / (c.high + 1);
        const score = open * 10 + c.low;
        if (score > bestScore) {
          bestScore = score;
          best = { x: c.sx / c.n, z: c.sz / c.n };
        }
      }
      if (best) return best;
    ...
    _placeBakedSpawnProps: function (spawn) {
      if (!spawn) return;
      const x = spawn.x;
      const z = spawn.z;
      const move = (id, dx, dy, dz) => {
        const el = document.getElementById(id);
        if (el) el.object3D.position.set(x + dx, dy, z + dz);
      };
      move('vr-height-panel', 0, 1.5, -1.55);
      move('mirror-body', 0, 0, -2);
      move('mirror-body-label', 0, 2.5, -2);

      const dummy = document.getElementById('grab-dummy');
      if (dummy) {
        const nx = x + 1.4;
        const ny = 0;
        const nz = z - 1.6;
        dummy.object3D.position.set(nx, ny, nz);
        const comp = dummy.components?.['grabbable-ragdoll'];
        if (comp) {
          comp.data.x = nx;
          comp.data.y = ny;
          comp.data.z = nz;
          if (comp._entityBasePos) comp._entityBasePos.set(nx, ny, nz);
          if (comp._staticPose?.entityPos) comp._staticPose.entityPos.set(nx, ny, nz);
        }
      }
    },
    ...
    _computeGroundFollowDy: function (cur, dt, isMoving) {
      ...
      const probeY = cur.y + 1.2;
      let surfaceY = this.terrain.getHeightAtWorld(cur.x, cur.z, probeY);
      ...
      if (isMoving) {
        ...
        if (horizLen > 0.04) {
          ...
          const aheadY = this.terrain.getHeightAtWorld(ax, az, probeY);
