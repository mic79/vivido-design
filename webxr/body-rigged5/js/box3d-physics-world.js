...
(function () {
  ...
  class Box3DPhysicsWorld {
    ...
    async init(options) {
      ...
      if (options?.addGround !== false) {
        this._addGround(options?.groundHalfExtent ?? 20);
      }
      if (options?.skipSceneColliders !== true) {
    ...
    _addGround(halfExtent = 20) {
      const he = Math.max(1, halfExtent);
      const def = this.b3.b3DefaultBodyDef();
      def.position = { x: 0, y: -0.05, z: 0 };
      const body = this.b3.b3CreateBody(this.world, def);
      this.b3.b3CreateBoxShape(body, this._shapeDef(), he, 0.05, he);
    }
