...
AFRAME.registerComponent('advanced-bot', {
  ...
  init: function() {
    ...
    this.el.sceneEl.addEventListener('loaded', () => {
      // Only set primary ball refs for the primary bot, not for bot2
      // (bot2's refs are set by teams-mode.js to player1b/player2b)
      if (this.el.id !== 'bot2') {
  ...
  onCollision: function(evt) {
    ...
    var hitBallPlayer = collider && collider.getAttribute('simple-grab') && collider.getAttribute('simple-grab').player;
    var isBlueTeamBall = (hitBallPlayer === 'player2' || hitBallPlayer === 'player2b');
    if (isBlueTeamBall) {
      ...
      // Determine the correct target element for the impact effect
      var hitTargetId = (this.el.id === 'bot2') ? 'bot2-target' : 'bot-target';
      
      // Update score — pass the hit target ID so impact effect shows on correct bot
      const gameManager = this.el.sceneEl.querySelector('#game-manager');
      if (gameManager && gameManager.components['game-manager']) {
        gameManager.components['game-manager'].onBotHit(hitTargetId);
      }
  ...
  calculateBounceThrow: function(botPos, targetPos) {
    var wallX = (window.matchFormat === '2v2') ? 4 : 2;
    const bounceTypes = [
      { axis: 'x', value: wallX },    // Right wall
      { axis: 'x', value: -wallX },   // Left wall
  ...
  _findMostThreateningBall: function() {
    // In 2v2, check all blue balls for the most threatening incoming one
    if (window.matchFormat === '2v2') {
      var blueBalls = document.querySelectorAll('[simple-grab^="player: player2"]');
      var best = null;
      var bestArrival = Infinity;
      var self = this;
      blueBalls.forEach(function (ball) {
        var g = ball.components && ball.components['simple-grab'];
        if (!g || !g.body || g.isGrabbed) return;
        var bp = g.body.position;
        var bv = g.body.velocity;
        var speed = Math.sqrt(bv.x * bv.x + bv.y * bv.y + bv.z * bv.z);
        if (bv.z >= -2 || speed < 3 || bp.z <= self._homePos.z + 2) return;
        var t = (self._homePos.z - bp.z) / bv.z;
        if (t > 0 && t < bestArrival) {
          bestArrival = t;
          best = g;
        }
      });
    ...
    return this.playerBall ? this.playerBall.components['simple-grab'] : null;
  },

  _updateReaction: function(time) {
    var grab;
    if (window.matchFormat === '2v2') {
      grab = this._findMostThreateningBall();
      if (!grab && this._reactState === 'idle') return;
      if (!grab) grab = this.playerBall && this.playerBall.components['simple-grab'];
    } else {
      if (!this.playerBall) return;
      grab = this.playerBall.components['simple-grab'];
    }
    ...
    switch (this._reactState) {
      ...
      case 'perceiving':
        ...
        // Only primary bot drives the shared hand-reaction target
        // (bot2's body is driven via remoteHandData in tick2v2)
        var _isPrimaryBot = (this.el.id !== 'bot2');
        if (_isPrimaryBot) {
      ...
      case 'executing':
        ...
        if (this.el.id !== 'bot2' && window.botReactionTarget) {
  ...
  throwBall: function() {
    ...
    if (arenaTarget) {
    ...
    } else if (window.matchFormat === '2v2') {
      // In 2v2, pick a random blue team target to throw at
      var targets = [];
      ...
      var bt = window.teamsMode && window.teamsMode.getBlueTeammateState();
      if (bt) {
        targets.push(bt.currentPos.clone());
      }
      if (targets.length === 0) return;
      playerPos = targets[Math.floor(Math.random() * targets.length)];
      playerPos.y += 0.5;
    } else {
    ...
    var ballPlayerSlot = this.ball.components['simple-grab'].data.player;
    const maxSpd = window.getStageMaxSpeed
      ? window.getStageMaxSpeed((window.playerStages || {})[ballPlayerSlot] || 1)
      : rawForce;
