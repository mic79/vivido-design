...
(function () {
  ...
  function getSpectatorPositions() {
    var offset = (window.matchFormat === '2v2') ? 6 : 4;
    return [
      { x: -offset, y: 0, z: 3, ry: -53 },
      { x: offset, y: 0, z: 3, ry: 53 },
      { x: -offset, y: 0, z: -3, ry: -127 },
      { x: offset, y: 0, z: -3, ry: 127 }
    ];
  }
  var SPECTATOR_POSITIONS = getSpectatorPositions();
  ...
  AFRAME.registerComponent('vr-menu', {
    ...
    handleMenuClick: function (buttonId) {
      switch (buttonId) {
        ...
        case 'menu-format-toggle': this.toggleMatchFormat(); break;
    ...
    setMode: function (mode) {
      ...
      var formatSection = document.getElementById('menu-format-section');
      if (formatSection) formatSection.setAttribute('visible', mode !== 'stats');
    ...
    toggleMatchFormat: function () {
      var current = window.matchFormat || '1v1';
      var next = (current === '1v1') ? '2v2' : '1v1';

      // End any active match before switching
      var gmEl = document.querySelector('#game-manager');
      var gm = gmEl && gmEl.components['game-manager'];
      if (gm && (gm.matchState === 'PLAYING' || gm.matchState === 'OVERTIME' || gm.matchState === 'COUNTDOWN')) {
        gm.endMatch(true);
      ...
      // Update visuals immediately (before activation which may take time)
      var formatText = document.getElementById('menu-format-text');
      var formatBtn = document.getElementById('menu-format-toggle');
      if (formatText) formatText.setAttribute('text', 'value', next.toUpperCase());
      if (formatBtn) setButtonColor(formatBtn, next === '2v2' ? '#ff8800' : '#555555');

      // Activate/deactivate 2v2 mode (arena scaling, entity creation)
      try {
        if (next === '2v2' && window.teamsMode) {
          window.teamsMode.activate2v2();
        } else if (next === '1v1' && window.teamsMode) {
          window.teamsMode.deactivate2v2();
        }
      } catch (e) {
        console.error('[Menu] Error switching match format:', e);
        window.matchFormat = next;
      }

      // Update lobby state if in multiplayer
      if (window.lobbyState) {
        window.lobbyState.matchFormat = next;
        if (typeof window.broadcastLobbyState === 'function') window.broadcastLobbyState();
      }

      this.updateMenuDisplay();
      refreshRaycasters();
      console.log('[Menu] Match format switched to ' + next);
    },
    ...
    startSingleplayerMatch: function () {
      ...
      if (gmEl && gmEl.components['game-manager']) {
        ...
        if (gm.matchState === 'PLAYING' || gm.matchState === 'OVERTIME' || gm.matchState === 'COUNTDOWN') {
          gm.endMatch(true);
          // If ending a 2v2 match, deactivate 2v2 entities are left in place for next match
          this.updateMenuDisplay();
          return;
        }
        if (gm.matchState === 'WAITING' || gm.matchState === 'ENDED') {
          ...
          // If 2v2 format is active, ensure all entities are set up
          if (window.matchFormat === '2v2' && window.teamsMode) {
            window.teamsMode.activate2v2();
          }
    ...
    moveToSpectator: function (slotIndex) {
      var positions = getSpectatorPositions();
      if (slotIndex >= 0 && slotIndex < positions.length) {
        this.movePlayerToPosition(positions[slotIndex]);
    ...
    moveToMatchPosition: function () {
      var pos = (window.matchFormat === '2v2') ? { x: -2, y: 0, z: 6 } : PLAYER_POSITION_BLUE;
      this.movePlayerToPosition(pos);
      var rig = document.getElementById('rig');
    ...
    updateMenuDisplay: function () {
      ...
      // Format toggle visibility (visible in single and multi modes)
      var formatSection = document.getElementById('menu-format-section');
      if (formatSection) formatSection.setAttribute('visible', this.menuMode !== 'stats');
      var formatText = document.getElementById('menu-format-text');
      var formatBtn = document.getElementById('menu-format-toggle');
      if (formatText) formatText.setAttribute('text', 'value', (window.matchFormat || '1v1').toUpperCase());
      if (formatBtn) setButtonColor(formatBtn, window.matchFormat === '2v2' ? '#ff8800' : '#555555');
      ...
      if (!ls) {
        ...
        if (startMatchBtn) {
          ...
          if (showStart) {
            ...
            var label = isPlaying ? 'END MATCH' : (window.matchFormat === '2v2' ? 'START 2v2' : 'START MATCH');
            if (stxt) stxt.setAttribute('text', 'value', label);
