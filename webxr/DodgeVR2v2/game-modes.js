...
AFRAME.registerComponent('game-modes', {
  ...
  setOpposingWallToBounce: function() {
    setTimeout(() => {
      ...
      this.respawningWall = document.getElementById('arena-back-wall');
      ...
      const leftWall = document.getElementById('arena-left-wall');
      if (leftWall) {
        this.bouncingWall = leftWall.cloneNode(true);
        this.bouncingWall.setAttribute('position', '0 2 -8');
        this.bouncingWall.setAttribute('width', (window.matchFormat === '2v2') ? '8' : '4');
    ...
    setTimeout(() => {
      ...
      playerBall.addEventListener('obbcollisionstarted', (evt) => {
        ...
        if (collider && collider.getAttribute && collider.getAttribute('geometry')) {
          ...
          if (pos && geom) {
            var _wallX = (window.matchFormat === '2v2') ? 4 : 2;
            const isSideWall = (Math.abs(pos.x - _wallX) < 0.01 || Math.abs(pos.x + _wallX) < 0.01) && geom.width === 0.1;
