...
AFRAME.registerComponent('grabbable-ragdoll', {
  schema: {
    modelPath: { type: 'string', default: 'Stand_Up_mixamo.fbx' },
    echoTeam: { type: 'string', default: 'orange' },
  ...
  init: function () {
    ...
    this._fbxUnitScale = 1;
    this._loadWhenReady();
  },

  update: function (oldData) {
    if (!oldData || oldData.modelPath === undefined) return;
    if (oldData.modelPath === this.data.modelPath && oldData.echoTeam === this.data.echoTeam) {
      return;
    }
  ...
  _isEchoModel: function (path) {
    return /Stand_Up_mixamo\.fbx$/i.test(path || this.data.modelPath || '');
  },

  _playerMixamoBody: function () {
    return document.querySelector('#local-body')?.components?.['mixamo-body'] || null;
  },
  ...
  _unloadModel: function () {
    ...
    if (this.modelLoaded && this.model) {
      try { this.resetRagdoll(); } catch (_) {}
    }

    if (this._idleMixer) {
      try { this._idleMixer.stopAllAction(); } catch (_) {}
    }
    this._idleMixer = null;
    ...
    if (this.model) {
      ...
      this.model.traverse((node) => {
        ...
        for (const mat of mats) {
          if (!mat) continue;
          // Echo PBR maps are cached/shared — do not dispose them on swap.
          mat.map = null;
          mat.normalMap = null;
    ...
    this.bones = {};
    this._skinnedMeshes = [];
    this._staticPose = null;
    this._bindPose = null;
    this._hipsRestPos = null;
    this._fbxUnitScale = 1;
    this._shatterBucketCache = { buckets: null, ready: false };
    this._shatterCachePending = false;
    this._echoHoldArmRest = false;
    this._echoPlantLegs = false;
    this._echoFootPlantL = null;
    this._echoFootPlantR = null;
  ...
  _loadModel: function () {
    ...
    if (!Loaders?.ready) {
      setTimeout(() => {
        if (token === this._modelLoadToken) this._loadModel();
      }, 50);
      return;
    }
    ...
    if (isGlb) {
      new Loaders.GLTFLoader().load(
        path,
        (gltf) => {
          if (token !== this._modelLoadToken) return;
          this._onModelLoaded(gltf.scene, gltf.animations || [], { isGltf: true, token });
        },
        undefined,
        (err) => console.error('[grabbable-ragdoll] GLB load error:', err)
      );
      return;
    }
    ...
    new Loaders.FBXLoader().load(
      ...
      (fbx) => {
        ...
        const finish = () => {
          if (token !== this._modelLoadToken) return;
          this._onModelLoaded(fbx, fbx.animations || [], { isGltf: false, token });
        };
        if (this._isEchoModel(path)) {
          const mb = this._playerMixamoBody();
          const loadTex = mb?._loadEchoTextures?.bind(mb)
            || (() => Promise.reject(new Error('no echo tex helper')));
          loadTex(this.data.echoTeam || 'orange').then((pack) => {
            if (token !== this._modelLoadToken) return;
            if (mb?._applyEchoMaterials) mb._applyEchoMaterials(fbx, pack);
            finish();
          }).catch(() => finish());
          return;
        }
      ...
      undefined,
      (err) => console.error('[grabbable-ragdoll] FBX load error:', err)
    );
  },
  ...
  _initIdleAnimation: function (animations) {
    ...
    // Echo Stand_Up embeds "mixamo.com" (stand-up), not Idle — don't loop that.
    if (!clip && !this._isEchoModel(this.data.modelPath)) clip = animations[0];
    ...
    action.time = 0;
    // play() before update — otherwise scale tracks never bind and static pose
    // snapshots Mixamo bind hips.scale=0.01 (Armature is already 0.01 → crush on restore).
    action.play();
    mixer.update(0);
  ...
  /** Retarget via mixamo-body sanitize (quat limbs + hips hold) — never raw YBot tracks. */
  _retargetIdleViaMixamoBody: function (sourceClip) {
    const mb = this._playerMixamoBody();
    if (!mb?._retargetClipToSkeleton || !sourceClip || !this.skeleton) return null;
    const proxy = Object.create(mb);
    proxy.skeleton = this.skeleton;
    proxy.bones = this.bones;
    proxy.model = this.model;
    proxy._bindPose = this._bindPose;
    return mb._retargetClipToSkeleton.call(
      proxy,
      sourceClip.clone(),
      'Idle',
      { mode: 'locomotion' }
  ...
  /**
   * Echo FBX has no embedded Idle. Retarget YBot Idle for upper body only —
   * full YBot leg tracks skate Echo feet in XZ. Legs stay on bind (planted);
   * arms/spine idle like YBot.
   */
  _stripEchoIdleLegTracks: function (clip) {
    if (!clip?.tracks) return clip;
    const normalize = (n) => n.replace(/mixamorig\d*/i, '').replace(/[:_\s|]/g, '').toLowerCase();
    const legRe = /^(left|right)(upleg|leg|foot|toebase|toeend|foottoebase)/;
    clip.tracks = clip.tracks.filter((track) => {
      const dot = track.name.lastIndexOf('.');
      if (dot < 0) return false;
      const bone = normalize(track.name.substring(0, dot));
      return !legRe.test(bone);
    });
    return clip;
  },

  _restoreEchoLegBind: function () {
    if (!this._bindPose || !this.skeleton) return;
    const keys = [
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (!bone) continue;
      const idx = this.skeleton.bones.indexOf(bone);
      const snap = idx >= 0 ? this._bindPose[idx] : null;
      if (!snap) continue;
      bone.quaternion.copy(snap.quat);
      bone.position.copy(snap.pos);
      bone.scale.copy(snap.scale);
    }
  },

  _captureEchoFootPlants: function () {
    if (!this.bones.leftFoot || !this.bones.rightFoot) return;
    const l = new THREE.Vector3();
    const r = new THREE.Vector3();
    this.bones.leftFoot.getWorldPosition(l);
    this.bones.rightFoot.getWorldPosition(r);
    this._echoFootPlantL = l;
    this._echoFootPlantR = r;
  },

  _loadEchoIdleFromYbot: function (token) {
    const apply = (pack) => {
      if (token !== this._modelLoadToken || !this.model || !this.skeleton) return;

      if (this.skeleton.pose) this.skeleton.pose();
      this.skeleton.update();
      this.model.updateMatrixWorld(true);
      if (!this._bindPose) this._captureBindPose();
      this._echoHoldArmRest = false;
      this._echoPlantLegs = false;

      if (!pack?.idle) {
        this._plantSolesToFloor();
        this._captureStaticPose();
        console.log('[grabbable-ragdoll] Echo NPC bind pose (no idle pack)');
        return;
      }

      const idle = this._retargetIdleViaMixamoBody(pack.idle);
      if (!idle) {
        this._plantSolesToFloor();
        this._captureStaticPose();
        console.warn('[grabbable-ragdoll] Echo idle retarget unavailable — bind pose only');
        return;
      }

      this._stripEchoIdleLegTracks(idle);
      const mb = this._playerMixamoBody();
      if (mb?._addIdleHipsHold) {
        const proxy = Object.assign(Object.create(mb), {
          skeleton: this.skeleton,
          bones: this.bones,
          _bindPose: this._bindPose
        });
        mb._addIdleHipsHold.call(proxy, idle, { includePosition: true });
      }

      if (this._initIdleAnimation([idle])) {
        this._restoreEchoLegBind();
        this._plantSolesToFloor();
        this._captureStaticPose();
        this._echoPlantLegs = true;
        this._captureEchoFootPlants();
        console.log(
          '[grabbable-ragdoll] Echo idle = YBot upper-body + planted bind legs',
          idle.tracks.length,
          'tracks'
        );
      } else {
    ...
    if (window._ybotLocoClipCache) {
      apply(window._ybotLocoClipCache);
      return;
    }
    if (window._ybotLocoClipWaiters) {
      window._ybotLocoClipWaiters.push(apply);
      return;
    }
    window._ybotLocoClipWaiters = [apply];
    const startLoad = () => {
      const Loaders = window.BodyRiggedLoaders;
      if (!Loaders?.ready || !Loaders.GLTFLoader) {
        ...
        (gltf) => {
          ...
          const pick = (...names) => {
            for (const n of names) {
              const c = THREE.AnimationClip.findByName(anims, n);
              if (c) return c;
            }
            return null;
          };
          const pack = {
            idle: pick('Idle', 'idle'),
            walk: pick('Walk', 'walk', 'Walking'),
            run: pick('Sprint', 'Run', 'run', 'Jog')
          };
          window._ybotLocoClipCache = pack;
          (window._ybotLocoClipWaiters || []).forEach((fn) => fn(pack));
          window._ybotLocoClipWaiters = null;
        },
        undefined,
        () => { window._ybotLocoClipWaiters = null; }
      );
  ...
  _normalizeFbxUnitScale: function (modelRoot) {
    const THREE = window.AFRAME.THREE;
    const YBOT_STAND_M = 1.657;
    modelRoot.scale.set(1, 1, 1);
    modelRoot.updateMatrixWorld(true);
    if (this.skeleton?.pose) {
      this.skeleton.pose();
      this.skeleton.update();
    } else {
      this.skeleton?.update?.();
    ...
    else if (this.bones.hips) this.bones.hips.getWorldPosition(head);
    let feetY = head.y;
    if (this.bones.leftFoot && this.bones.rightFoot) {
      this.bones.leftFoot.getWorldPosition(footL);
      this.bones.rightFoot.getWorldPosition(footR);
      feetY = Math.min(footL.y, footR.y);
    }
    const boneH = Math.max(0.001, head.y - feetY);
    const box = new THREE.Box3().setFromObject(modelRoot);
    const solePad = Math.max(0, feetY - box.min.y);
    const h = boneH + solePad;
    this._fbxUnitScale = YBOT_STAND_M / h;
    modelRoot.scale.setScalar(this._fbxUnitScale);
    modelRoot.updateMatrixWorld(true);
    this.skeleton?.update?.();
  },

  _alignFbxModelToOrigin: function (modelRoot) {
    const THREE = window.AFRAME.THREE;
    ...
    this.bones.hips.getWorldPosition(hipsW);

    // Prefer toe/foot bones over mesh AABB — Echo's skinned box.min can sit on the
    // floor from non-foot verts while boots (foot-weighted) float after idle.
    const tmp = new THREE.Vector3();
    let feetY = Infinity;
    const consider = (bone, pad) => {
      if (!bone) return;
      bone.getWorldPosition(tmp);
      feetY = Math.min(feetY, tmp.y - pad);
    };
    consider(this.bones.leftToe, 0.008);
    consider(this.bones.rightToe, 0.008);
    if (!Number.isFinite(feetY) || feetY === Infinity) {
      consider(this.bones.leftFoot, this.ankleToSoleM || 0.08);
      consider(this.bones.rightFoot, this.ankleToSoleM || 0.08);
    }
    if (!Number.isFinite(feetY) || feetY === Infinity) {
      const box = new THREE.Box3().setFromObject(modelRoot);
      feetY = box.min.y;
    }
    if (!Number.isFinite(feetY)) feetY = hipsW.y;

    const parent = modelRoot.parent;
    const hipsLocal = hipsW.clone();
    const feetLocal = new THREE.Vector3(hipsW.x, feetY, hipsW.z);
    if (parent) {
      parent.worldToLocal(hipsLocal);
      parent.worldToLocal(feetLocal);
    }

    modelRoot.position.x -= hipsLocal.x;
    modelRoot.position.z -= hipsLocal.z;
    modelRoot.position.y -= feetLocal.y;
    modelRoot.updateMatrixWorld(true);
    this.skeleton?.update?.();
  },

  /**
   * Keep boot soles on the floor. YBot idle on Echo moves foot bones up while hips
   * stay held — mesh AABB can still read y=0 from other verts, so boots float.
   */
  _plantSolesToFloor: function () {
    if (!this.model || !this.bones.leftFoot) return;
    ...
    const consider = (bone, pad) => {
      if (!bone) return;
      bone.getWorldPosition(tmp);
      lowestToe = Math.min(lowestToe, tmp.y - pad);
    };
    consider(this.bones.leftToe, 0.008);
    consider(this.bones.rightToe, 0.008);
    if (!Number.isFinite(lowestToe) || lowestToe === Infinity) {
      consider(this.bones.leftFoot, this.ankleToSoleM || 0.08);
      consider(this.bones.rightFoot, this.ankleToSoleM || 0.08);
    }
    if (!Number.isFinite(lowestToe) || lowestToe === Infinity) return;

    const target = (this.FLOOR_Y || 0) + 0.002;
    const dy = target - lowestToe;
    if (Math.abs(dy) < 0.0005) return;
    this.model.position.y += dy;
    this.model.updateMatrixWorld(true);
  },

  _captureStaticPose: function () {
  ...
  /** Echo FBX bind rolls ≠ YBot — bone-local XYZ tip makes the whole body go diagonal. */
  _usesWorldSpaceHitReaction: function () {
    return !!this._echoPlantLegs;
  },

  _hasActiveHitReaction: function () {
    const rn = this._rootNudge;
    if (rn.pos.lengthSq() > 1e-8 || rn.vel.lengthSq() > 1e-8) return true;
    if (Math.abs(rn.rotY) > 1e-5 || Math.abs(rn.angVelY) > 1e-5) return true;
  ...
  // YBot = body-rigged4 bone-local. Echo = world-space (same visual tip-back despite bind rolls).
  _addBoneHitImpulse: function (boneKey, shotDir, impactNormal, magnitude) {
    ...
    // Echo planted legs: hip twists skate the feet and never recover.
    if (boneKey === 'hips' && this._echoPlantLegs) return;

    const st = this._ensureHitBoneState(boneKey);
    const worldHit = this._usesWorldSpaceHitReaction();
    ...
    if (worldHit) {
      this._segAB.set(0, 1, 0).cross(shotDir);
      if (this._segAB.lengthSq() > 1e-8) {
        this._segAB.normalize();
        st.angVel.x += this._segAB.x * magnitude * 3.4;
        st.angVel.y += this._segAB.y * magnitude * 3.4;
        st.angVel.z += this._segAB.z * magnitude * 3.4;
      }
    } else {
  ...
  _addLimbPushImpulse: function (boneKey, pushDir, magnitude) {
    ...
    if (this._usesWorldSpaceHitReaction()) {
      st.angVel.x += this._tmpV.x * k;
      st.angVel.y += this._tmpV.y * k;
      st.angVel.z += this._tmpV.z * k;
      return;
    }
  ...
  _updateHitReactions: function (dt) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (worldHit) {
        ...
        if (speed * dt > 1e-7) {
          this._closestAxis.copy(av).multiplyScalar(1 / speed);
          this._hitDq.setFromAxisAngle(this._closestAxis, speed * dt);
          st.offsetQuat.premultiply(this._hitDq);
          st.offsetQuat.normalize();
        }
        // Cap stacked Echo head bends (world offsets compound harder than bone-local).
        const ow = st.offsetQuat.w;
        const ang = 2 * Math.acos(Math.min(1, Math.abs(ow)));
        const maxAng = (keys[i] === 'head' || keys[i] === 'neck') ? 0.85 : 1.15;
        if (ang > maxAng && ang > 1e-6) {
          st.offsetQuat.slerp(this._hitDq.identity(), 1 - maxAng / ang);
          st.offsetQuat.normalize();
        }
      } else {
        const ax = av.x * dt;
  ...
  _applyHitReactionsToPose: function () {
    ...
    if (worldHit && keys.length) {
      // Snapshot idle world quats first — in-place parent→child corrupts Echo lean.
      if (!this._hitIdleWorld) this._hitIdleWorld = {};
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (nearId) continue;
        if (!this._hitIdleWorld[boneKey]) this._hitIdleWorld[boneKey] = new THREE.Quaternion();
        bone.getWorldQuaternion(this._hitIdleWorld[boneKey]);
        active.push(boneKey);
      }
      ...
      for (let i = 0; i < active.length; i++) {
        const boneKey = active[i];
        const st = this._hitBoneState[boneKey];
        const bone = this.bones[boneKey];
        bone.parent.updateMatrixWorld(true);
        bone.parent.getWorldQuaternion(this._tmpQ);
        this._segQuat.copy(st.offsetQuat).multiply(this._hitIdleWorld[boneKey]);
        bone.quaternion.copy(this._tmpQ).invert().multiply(this._segQuat);
        bone.updateMatrixWorld(true);
      }

      // Re-plant feet after torso/head recoil — never during a leg flinch.
      if (this._echoPlantLegs && !this._shattered && !this._legHitOverlayActive()) {
        this._restoreEchoLegBind();
        if (this.model) this.model.updateMatrixWorld(true);
      }
    } else {
  ...
  _onModelLoaded: function (modelRoot, animations, meta) {
    meta = meta || { isGltf: true };
    if (meta.token != null && meta.token !== this._modelLoadToken) return;
    ...
    if (meta.isGltf) {
      ...
      modelRoot.traverse((n) => {
        if (n.isPoints || n.isLine || n.isLineSegments) n.visible = false;
      });
    }
    ...
    modelRoot.traverse((node) => {
      if (node.isMesh) {
        ...
        if (node.material && !this._isEchoModel(this.data.modelPath)) {
    ...
    if (!meta.isGltf) {
      this._normalizeFbxUnitScale(modelRoot);
      this._alignFbxModelToOrigin(modelRoot);
    }

    // Snapshot bind before any idle retarget (hips hold + crush recovery).
    if (this.skeleton?.pose) {
      this.skeleton.pose();
      this.skeleton.update();
    }
    ...
    const hasIdle = this._initIdleAnimation(animations || []);
    if (!hasIdle && this._isEchoModel(this.data.modelPath)) {
      this._loadEchoIdleFromYbot(meta.token || this._modelLoadToken);
    }
    ...
    console.log(
      '[grabbable-ragdoll] ready',
      meta.isGltf ? 'GLB' : 'FBX',
      this._isEchoModel(this.data.modelPath) ? '(Echo)' : '',
      'at',
      this.data.x,
      this.data.z
    );
  ...
  /**
   * Shatter bake frame (world matrix whose inverse maps verts into meter-ish local space).
   * YBot: model.matrixWorld — identical to body-rigged4 (model local ≈ meters).
   * Echo: entity.matrixWorld — FBX unit scale lives on the model child, so entity
   * local is already meters. Do NOT invent Echo-specific shatter thresholds.
   */
  _shatterBakeMatrixWorld: function () {
    if (this._isEchoModel(this.data.modelPath)) {
      const root = this.el.object3D;
      if (root) root.updateMatrixWorld(true);
      return root ? root.matrixWorld : null;
    }
  ...
  _prepareEntityForRagdollSpawn: function () {
    this._clearHitReactions();
    // Match body-rigged4 / YBot: do NOT _restoreStaticPose here. YBot's Mixamo bind
    // has hips.scale≈0.01 under Armature 0.01; restoring that crushes the mesh to a
    // speck (looks like the whole character vanishes on head/leg destroy → ragdoll).
    this.el.object3D.position.copy(this._entityBasePos);
    this.el.object3D.rotation.y = this._entityBaseRotY;
    this.el.object3D.updateMatrixWorld(true);
    if (this.skeleton) this.skeleton.update();
    if (this.model) this.model.updateMatrixWorld(true);
    // Echo-only: clear leftover hit-reaction / idle leg drift before calibrate.
    if (this._isEchoModel?.(this.data.modelPath)) {
      this._restoreEchoLegBind?.();
      this._plantSolesToFloor?.();
    }
  ...
  _spawnRagdoll: function (opts) {
    ...
    // Collapse = floppy free fall (same as body-rigged4 / YBot). Echo used to get
    // friction 0.28 + motors on, which froze the ragdoll early.
    // Do not alignBodyCapsuleAxesToMesh — any capsule swing pancaked Echo settle
    // (hipsY~0.12 vs YBot~0.56). Retarget uses the same body*offset path as YBot;
    // echoNoHingeRetarget avoids Echo arm spaghetti.
    const jointFriction = collapse
      ? 0.04
      : (opts.jointFriction != null ? opts.jointFriction : 0.14);
  ...
  _applyRagdollLegHitOverlay: function () {
    for (let i = 0; i < LEG_BONE_KEYS.length; i++) {
      ...
      bone.updateMatrixWorld(true);
      bone.parent.getWorldQuaternion(this._tmpQ);
      bone.getWorldQuaternion(this._hitDq);
      this._segQuat.copy(st.offsetQuat).multiply(this._hitDq);
      bone.quaternion.copy(this._tmpQ).invert().multiply(this._segQuat);
  ...
  _regionIdFromHit: function (mesh, hit) {
    ...
    if (regionId && RS.CORE_TORSO_REGIONS?.[regionId] && RS.resolveHitRegionAtPoint
      && hit.point) {
      const bakeMW = this._shatterBakeMatrixWorld();
      if (!bakeMW) return regionId;
      const Mat4 = mesh.matrixWorld.constructor;
      const spaceInverse = new Mat4().copy(bakeMW).invert();
  ...
  _bakeShardsToWorld: function (newShards) {
    if (!newShards?.length) return;
    const bakeMW = this._shatterBakeMatrixWorld();
    if (!bakeMW) return;
    for (let i = 0; i < newShards.length; i++) {
      const mesh = newShards[i].mesh;
      if (mesh) mesh.position.applyMatrix4(bakeMW);
    }
  },
  ...
  tick: function (time, deltaTime) {
    ...
    if (!this.ragdollActive) {
      ...
      if (this._idleMixer) {
        ...
        if (this.skeleton) this.skeleton.update();
        // Echo idle strips leg tracks — without a per-frame bind reset, hit-reaction
        // world offsets compound into the local quats (legs fold into a mid-air V).
        // Always restore bind legs first; hit overlay applies on top of that base.
        if (this._echoPlantLegs && !this._shattered) {
          this._restoreEchoLegBind();
        }
      ...
      if (this.skeleton) this.skeleton.update();
      if (this.model) this.model.updateMatrixWorld(true);
      // Sole plant after recoil — skipped while bones are actively flinching.
      if (this._echoPlantLegs && !this._shattered && !this._hasActiveHitReaction()) {
        this._plantSolesToFloor();
      }
