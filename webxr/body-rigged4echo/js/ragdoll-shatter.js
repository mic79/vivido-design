...
(function () {
    ...
    return {
      ...
      resolveMaterial: function (mesh, matIdx) {
        ...
        const src = Array.isArray(mesh.material)
          ...
          : mesh.material;
        if (!src) return null;
        // YBot GLB shards share unmapped MeshPhysical (cyan plastic, metalness 0).
        // Echo FBX shares MeshStandard + maps → painted suit fragments. Force solid
        // Physical debris — black for Echo (not YBot cyan).
        const textured = !!(src.map || src.normalMap || src.metalnessMap || src.roughnessMap
          || src.type === 'MeshStandardMaterial' || src.type === 'MeshPhongMaterial');
        if (!textured) return src;
        if (src.userData && src.userData._shatterDebrisMat) {
          return src.userData._shatterDebrisMat;
        }
        const THREE = window.AFRAME.THREE;
        ...
        mat.userData.isShatterDebrisMat = true;
        if (!src.userData) src.userData = {};
        src.userData._shatterDebrisMat = mat;
        return mat;
      }
    };
  }
  ...
  function convexMeshFromBucket(bucket, factory) {
    ...
    try {
      ...
      console.warn('[ragdoll-shatter] convex hull failed for', bucket.id, err);
      return null;
    }
    geo.computeVertexNormals();
