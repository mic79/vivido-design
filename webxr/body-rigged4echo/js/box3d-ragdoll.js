...
(function () {
  ...
  const _rbMatA = new THREE.Matrix4();
  const _rbMatB = new THREE.Matrix4();
  const _rbMatJoint = new THREE.Matrix4();
  const _rbMatLocalA = new THREE.Matrix4();
  const _rbMatLocalB = new THREE.Matrix4();
  const _rbQ = new THREE.Quaternion();
  const _rbP = new THREE.Vector3();
  const _rbS = new THREE.Vector3(1, 1, 1);

  /**
   * After bodies are rotated (e.g. Echo capsule-axis align), createHuman joint
   * local frames fight the new poses and freeze the ragdoll. Destroy joints and
   * recreate them so the CURRENT body pose is the zero-error rest pose:
   * keep localFrameA, set localFrameB = inv(bodyB) * bodyA * localFrameA.
   */
  function rebuildHumanJointsAtCurrentPose(b3, world, human, friction, hertz, damping, opts) {
    if (!b3 || !world || !human?.bodies || !human.joints) return;
    friction = friction ?? 0.05;
    hertz = hertz ?? 0;
    damping = damping ?? 0.5;
    opts = opts || {};
    const enableJointMotors = opts.enableJointMotors !== false && friction > 0;
    const floppyLimbs = opts.floppyLimbs === true;
    const dynamicFlags = human.dynamicFlags || null;
    ...
    for (let i = 1; i < BONES.length; i++) {
      const bone = BONES[i];
      if (!bone.joint) continue;
      if (dynamicFlags && !dynamicFlags[i] && !dynamicFlags[bone.parent]) continue;
      const j = bone.joint;
      const bodyA = human.bodies[bone.parent];
      const bodyB = human.bodies[i];
      if (!bodyA || !bodyB) continue;

      const posA = b3.b3Body_GetPosition(bodyA);
      const rotA = b3.b3Body_GetRotation(bodyA);
      const posB = b3.b3Body_GetPosition(bodyB);
      const rotB = b3.b3Body_GetRotation(bodyB);
      _rbQ.set(rotA.v.x, rotA.v.y, rotA.v.z, rotA.s);
      _rbP.set(posA.x, posA.y, posA.z);
      _rbMatA.compose(_rbP, _rbQ, _rbS);
      _rbQ.set(rotB.v.x, rotB.v.y, rotB.v.z, rotB.s);
      _rbP.set(posB.x, posB.y, posB.z);
      _rbMatB.compose(_rbP, _rbQ, _rbS);

      const lfA = j.localFrameA;
      _rbQ.set(lfA.q.v.x, lfA.q.v.y, lfA.q.v.z, lfA.q.s);
      _rbP.set(lfA.p.x, lfA.p.y, lfA.p.z);
      _rbMatLocalA.compose(_rbP, _rbQ, _rbS);
      _rbMatJoint.multiplyMatrices(_rbMatA, _rbMatLocalA);
      _rbMatLocalB.copy(_rbMatB).invert().multiply(_rbMatJoint);
      _rbMatLocalB.decompose(_rbP, _rbQ, _rbS);

      const frameA = {
        p: { x: lfA.p.x, y: lfA.p.y, z: lfA.p.z },
        q: normQ(lfA.q.v, lfA.q.s)
      };
      const frameB = {
        p: { x: _rbP.x, y: _rbP.y, z: _rbP.z },
        q: normQ({ x: _rbQ.x, y: _rbQ.y, z: _rbQ.z }, _rbQ.w)
      };
      ...
      if (j.type === 'revolute') {
        ...
        const motorScale = isWrist ? 2.8 : 1.0;
        def.maxMotorTorque = enableJointMotors ? j.friction * friction * motorScale : 0;
        human.joints.push(b3.b3CreateRevoluteJoint(world, def));
      } else {
        const def = b3.b3DefaultSphericalJointDef();
        def.base.bodyIdA = bodyA;
        def.base.bodyIdB = bodyB;
        ...
        def.enableMotor = enableJointMotors;
        def.maxMotorTorque = enableJointMotors ? j.friction * friction : 0;
        human.joints.push(b3.b3CreateSphericalJoint(world, def));
      }
    }

    if (!dynamicFlags || dynamicFlags[6] || dynamicFlags[8]) {
      const filterDef = b3.b3DefaultFilterJointDef();
      filterDef.base.bodyIdA = human.bodies[6];
      filterDef.base.bodyIdB = human.bodies[8];
      human.joints.push(b3.b3CreateFilterJoint(world, filterDef));
    }
  }
  ...
  window.Box3DRagdoll = {
    ...
    createHuman,
    destroyHuman,
    rebuildHumanJointsAtCurrentPose,
    computeLowestY,
    translateHuman,
    rotateHumanYaw,
    setHumanVelocity,
