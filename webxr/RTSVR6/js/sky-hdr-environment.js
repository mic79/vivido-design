...
/** Same path as on disk: `RTSVR4/assets/earthlike_planet.hdr` when served from the RTSVR4 root. */
...
export async function applyHdrSkyEnvironment(sceneEl, relativePath = DEFAULT_HDR_REL) {
  ...
  if (!THREE || !renderer || !scene || !THREE.PMREMGenerator) {
    console.warn('RTSVR4: HDR sky skipped (THREE / renderer / PMREM missing)');
    return;
  }
  ...
  try {
    ...
    console.warn('RTSVR4: HDR fetch failed', relativePath, e);
    return;
  }
  ...
  try {
    ...
    console.warn('RTSVR4: HDR parse failed', e);
    return;
  }
  ...
  try {
    ...
    console.warn('RTSVR4: PMREM from HDR failed', e);
    eq.dispose();
    pmrem.dispose();
    return;
  }
