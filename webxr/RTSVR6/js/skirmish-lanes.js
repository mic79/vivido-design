...
/**
 * Valley / X-canyon cliff docs. Visual walls live in the UE-exported GLB
 * (`export/skirmish-1v1-barriers.json` → add_skirmish_symmetric_barriers.py).
 * Keep empty here so we never double-draw or stamp fake nav.
...
export function buildSkirmishValleyMeshes(THREE, propsTemplate) {
  ...
  if (!cliffs.length) {
    console.warn('[RTSVR6] valley: no Cliff meshes in props template — skip walls');
    return null;
  }
