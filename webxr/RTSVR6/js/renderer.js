...
const MSAA4X_PREF_KEY = 'rtsvr6-msaa4x';
const MSAA4X_PREF_KEY_LEGACY = 'rtsvr5-msaa4x';
const MSAA4X_RELOAD_KEY = 'rtsvr6-msaa4x-reloaded';
...
function readMsaa4xPref() {
  try {
    ...
    if (desktop) {
      ...
      try {
        if (sessionStorage.getItem('rtsvr6-msaa4x-session') === '1') {
          const v = localStorage.getItem(MSAA4X_PREF_KEY) ?? localStorage.getItem(MSAA4X_PREF_KEY_LEGACY);
    ...
    if (v === '0' || v === 'off' || v === 'false') return false;
    if (v === '1' || v === 'on' || v === 'true') return true;
    const legacy = localStorage.getItem(MSAA4X_PREF_KEY_LEGACY);
    if (legacy === '0' || legacy === 'off' || legacy === 'false') return false;
    if (legacy === '1' || legacy === 'on' || legacy === 'true') return true;
  } catch (_) {
    /* private mode */
  }
...
function applyDesktopXrFramebufferScale(renderer, reason) {
  ...
  console.log('[RTSVR6] XR framebuffer scale', scale, reason || '');
}
    ...
    const syncXrPresent = () => {
      ...
      try {
        ...
        if (attrs && attrs.antialias && skirmishKitKind() !== 'rocks') {
          console.warn(
            '[RTSVR6] WebGL antialias/MSAA is ON during XR — intro/match will tank. Reload with MSAA off (?msaa=0).'
          );
        }
      } catch (_) {
        /* */
...
export function setMsaa4xEnabled(on) {
  _msaa4xOn = !!on;
  try {
  ...
  try {
    if (_msaa4xOn) sessionStorage.setItem('rtsvr6-msaa4x-session', '1');
    else sessionStorage.removeItem('rtsvr6-msaa4x-session');
  } catch (_) {
    /* ignore */
  }
  if (typeof window !== 'undefined') window.__rtsMsaa4x = _msaa4xOn;
  if (typeof UI.syncMsaa4xToggleUi === 'function') UI.syncMsaa4xToggleUi();
  applyMsaa4xGpuState();
  return _msaa4xOn;
}

export function toggleMsaa4x() {
  return setMsaa4xEnabled(!_msaa4xOn);
}

/** Canvas / XR MSAA is a context-creation flag — reload once if the live GL context disagrees. */
function applyMsaa4xGpuState() {
  const sceneEl = typeof document !== 'undefined' ? document.getElementById('game-scene') : null;
  const renderer = sceneEl && sceneEl.renderer;
  ...
  if (!attrs) return;
  const have = !!attrs.antialias;
  if (have === _msaa4xOn) {
    try {
      sessionStorage.removeItem(MSAA4X_RELOAD_KEY);
    } catch (_) {
      /* ignore */
    }
    return;
  ...
  try {
    if (sessionStorage.getItem(MSAA4X_RELOAD_KEY) === (_msaa4xOn ? '1' : '0')) return;
    sessionStorage.setItem(MSAA4X_RELOAD_KEY, _msaa4xOn ? '1' : '0');
  } catch (_) {
...
function installRenderGate(sceneEl) {
  ...
  r.render = function (scene, camera) {
    if (sceneEl.__rtsSkipRender && !window.__rtsForceRender) return;
    try {
      if (typeof window.__rtsUpdateKitLod === 'function') window.__rtsUpdateKitLod(camera);
    } catch (_) {
      /* kit not ready */
    }
    orig(scene, camera);
