...
/** BattleVR-style: host id is `RTSVR6-host-{N}` so joiners only pick lobby 1–4 (no pasted PeerJS id). */
...
function hostSessionId() {
  return `RTSVR6-host-${selectedLobby}`;
}
...
export async function joinGame() {
  ...
  const clientId = `RTSVR6-client-${Date.now().toString(36)}`;
