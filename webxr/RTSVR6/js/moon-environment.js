...
export async function ensureFocusCullPipeline(groundEl) {
  ...
  try {
    ...
    console.warn('[RTSVR6] ensureFocusCullPipeline lod failed', err);
    FogVisual.installFogVisualUnder(lodRoot);
  }
}
...
async function finishBakedMoonLook(THREE, sceneEl, root, opts = {}) {
  const recv =
    typeof window._getDynamicShadowsEnabled === 'function'
      ...
      : true;
  let cheap = false;
  let mesaHf = !!(root.userData && root.userData.rtsMesaHeightfield);
  root.traverse((obj) => {
    if (!obj.isMesh) return;
    if (obj.material && obj.material.userData && obj.material.userData.cheapMoonLook) cheap = true;
    if (obj.material && obj.material.userData && obj.material.userData.rtsMesaHeightfield) mesaHf = true;
    obj.receiveShadow = recv;
    obj.castShadow = false;
    if (obj.material && obj.material.userData) obj.material.userData.shadowRecv = recv;
  });
  // Never overwrite slope-vertex-color mesa plates with tiled moon albedo.
  if (!cheap && !mesaHf) {
  ...
  if (mesaHf) {
    try {
      await applyMesaHqTextures(root, THREE, sceneEl);
    } catch (err) {
...
function skirmishBakeMode() {
  // Lobby/intro: crater ridges. Match prepare/run: Hera Planum (or moononly).
  const inMatch =
    !!(State.gameSession && (State.gameSession.matchPreparing || State.gameSession.gameStarted));
  return inMatch ? 'match' : 'intro';
}

function skirmishBakeUrlMatchesWanted(root) {
  if (!root || !root.userData) return false;
  const got = root.userData.rtsBakeUrl || '';
  const want = preferredSkirmishBakeUrl(skirmishBakeMode());
  return !!got && got === want;
}
...
async function attachSkirmishSceneryProps(groundEl, sceneEl) {
  ...
  const inMatch =
    !!(State.gameSession && (State.gameSession.matchPreparing || State.gameSession.gameStarted));
  // Hera / mesa heightfield: never load quest rocks or crater Prop_* (wrong seating).
  const mesaHf = !!(
    bakedMoonRoot &&
    bakedMoonRoot.userData &&
    bakedMoonRoot.userData.rtsMesaHeightfield
  );
  let mode = !inMatch ? 'A0' : skirmishSceneryMode();
  if (mesaHf && mode !== 'A1') mode = 'A0';
  if (mode === 'A0') {
    if (bakedMoonRoot) setBakedMoonRockShadowsEnabled(bakedMoonRoot, false);
    clearSkirmishLaneRidges(groundEl);
  ...
  if (mode === 'A1') {
    ...
    // Never fall back to quest rocks on a mesa plate (handled above via A0).
    props = takeEmbeddedSkirmishProps();
    if (!props && !mesaHf) props = await tryLoadQuestRocksProps();
  }
  if (!props) {
    if (bakedMoonRoot) setBakedMoonRockShadowsEnabled(bakedMoonRoot, false);
    return false;
  ...
  try {
    ...
    console.warn('[RTSVR6] props instancing failed', err);
  }
  console.log('[RTSVR6] skirmish scenery props', {
    mode,
...
async function mountKitTerrain(groundEl, prev, sceneEl, kind) {
  ...
  if (prevKind === kind && wantLean === hadLean) {
    ...
    console.log('[RTSVR6] kit terrain (already loaded)', kind, { lean: hadLean });
    return true;
  }
  if (prevKind === kind && wantLean && !hadLean) {
    bakedMoonRoot = prev;
    ...
    syncTerrainGridHelperSize();
    await ensureFocusCullPipeline(groundEl);
    console.log('[RTSVR6] kit terrain lean applied in-place', kind);
    return true;
  }
  ...
  configureTerrainPresentation(sceneEl);
  syncTerrainGridHelperSize();
  console.log('[RTSVR6] kit terrain', {
    kind,
...
export async function dressKitGroundPlateWithMoon(kitRoot, sceneEl) {
  ...
  console.log('[RTSVR6] kit ground plate dressed with moon textures', { kind });
}
...
function mountLobbyKitPlate(groundEl, sceneEl) {
  ...
  configureTerrainPresentation(sceneEl);
  syncTerrainGridHelperSize();
  console.log('[RTSVR6] lobby ground (kit deferred until match start)');
  return true;
}
...
function applyNoTerrainIfRequested() {
  ...
  mesh.visible = false;
  console.log('[RTSVR6] noterrain: ground hidden');
}
...
async function applyMoonBattlefieldVisualsInner(sceneEl) {
  ...
  if (MAP_TERRAIN_STYLE === 'kit') {
    ...
    if (mountLobbyKitPlate(groundEl, sceneEl)) {
      styleMoonGrid();
      const gridMount = document.getElementById('gridHelper');
      if (gridMount && gridMount.object3D) {
        gridMount.object3D.traverse((o) => {
  ...
  const baked = await tryLoadBakedSkirmishMoon({ mode: skirmishBakeMode() });
  if (baked) {
...
async function rebuildMoonBattlefieldInner(sceneEl) {
  ...
  if (MAP_TERRAIN_STYLE === 'kit') {
    ...
    const ok = await mountKitTerrain(groundEl, prev, sceneEl, kind);
    if (ok) return;
    console.warn('[RTSVR6] kit terrain failed to load', kind);
  }
  ...
  if (bakedMoonAllowed() && parkedSkirmish && parkedSkirmish.root) {
    if (skirmishBakeUrlMatchesWanted(parkedSkirmish.root)) {
      ...
      if (restored) {
        ...
        syncTerrainGridHelperSize();
        await attachSkirmishSceneryProps(groundEl, sceneEl);
        console.log('[RTSVR6] restored parked skirmish moon', restored.userData?.rtsBakeUrl);
        return;
      }
    } else {
      // Parked crater while match wants Hera (or reverse) — do not restore the wrong plate.
      disposeGroundObject(parkedSkirmish.root);
      parkedSkirmish = null;
    }
  }

  // Already on the correct bake (e.g. rematch) — keep mesh, refresh props/nav cover.
  if (bakedMoonAllowed() && prevIsBake && skirmishBakeUrlMatchesWanted(prev)) {
    bakedMoonRoot = prev;
    horizonSkirtAttached = false;
    await finishBakedMoonLook(THREE, sceneEl, prev, { skipHeight: true });
    installGroundFogVisual(groundEl);
    configureTerrainPresentation(sceneEl);
    syncTerrainGridHelperSize();
    await attachSkirmishSceneryProps(groundEl, sceneEl);
    console.log('[RTSVR6] kept skirmish bake', prev.userData?.rtsBakeUrl);
    return;
  }
  ...
  if (baked) {
    ...
    syncTerrainGridHelperSize();
    await attachSkirmishSceneryProps(groundEl, sceneEl);
    console.log('[RTSVR6] loaded skirmish bake', baked.userData?.rtsBakeUrl, skirmishBakeMode());
    return;
  }
  ...
  const ok =
    kind === 'rocks'
      ...
      : kind === 'story-lean'
        ? got === 'story' && gotLean
        : got === 'story' && !gotLean;
  console.log('[RTSVR6] swapSkirmishKit done', { want: kind, got, lean: gotLean, ok });
  return ok;
}
