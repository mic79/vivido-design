...
function getKitGltfLoader() {
  ...
  if (!kitGltfLoader) {
    ...
    try {
      ...
      console.warn('[RTSVR6] DRACOLoader setup failed', err);
    }
...
function assembleKitWrap(gltf, opts) {
  ...
  if ((kind === 'overview' || asProps) && (keepNameRe || targetSpanM > 0)) {
    ...
    if (!seeded.any || seeded.cluster.isEmpty()) {
      console.warn('[RTSVR6] skip kit: no visible meshes', kind);
      return null;
    }
    ...
    if (!clusterAny || cluster.isEmpty()) {
      console.warn('[RTSVR6] skip kit: cluster empty', kind);
      return null;
    }
    y0 = waterY != null ? waterY : dirtFloorYFromMins(dirtMins, cluster.min.y);
    y0Source = waterY != null ? 'water' : 'dirt';
...
async function ensureStoryKitBuffer() {
  ...
  for (const url of urls) {
    ...
    if (next.byteLength < minBytes) {
      console.warn('[RTSVR6] skip story kit: file too small', url, next.byteLength);
      continue;
    }
    try {
      parseGlbJson(next);
    } catch (err) {
      console.warn('[RTSVR6] skip story kit: bad GLB', url, err);
      continue;
    }
    buf = next;
    used = url;
  ...
  glbBufCache = buf;
  glbBufUrl = used;
  console.log('[RTSVR6] story kit file', {
    url: used,
...
export async function tryLoadRocksKit() {
  ...
  if (!rocksBufCache) {
    ...
    if (!res.ok) {
      console.warn('[RTSVR6] skip rocks kit: missing', url);
      return null;
    }
    const next = await res.arrayBuffer();
    if (next.byteLength < MIN_STORY_ROCKS_BYTES) {
      console.warn('[RTSVR6] skip rocks kit: file too small', next.byteLength);
      return null;
    }
    try {
      parseGlbJson(next);
    } catch (err) {
      console.warn('[RTSVR6] skip rocks kit: bad GLB', err);
      return null;
    }
    rocksBufCache = next;
    console.log('[RTSVR6] rocks kit file', { url, bytes: next.byteLength });
  }
...
export async function tryLoadOverviewGroundscape() {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      if (ext.some((e) => /quantization/i.test(String(e)))) {
        console.warn('[RTSVR6] skip groundscape: quantized GLB (rebake required)', url);
        continue;
      }
    } catch {
      continue;
  ...
  if (!buf) {
    console.warn('[RTSVR6] overview groundscape missing — skirmish stays moon-only');
    return null;
  }

  console.log('[RTSVR6] overview groundscape', {
    url: used,
    bytes: buf.byteLength,
  });
  const gltf = await parseKitBuf(buf);
...
export async function tryLoadQuestRocksProps() {
  ...
  if (!buf) {
    console.warn('[RTSVR6] quest rocks props missing');
    return null;
  }

  console.log('[RTSVR6] quest rocks props file', { url: used, bytes: buf.byteLength });
  const gltf = await parseKitBuf(buf);
...
export async function tryLoadOverviewKit() {
  ...
  if (!overviewBufCache || overviewBufIsFull !== full) {
    ...
    if (!buf) {
      console.warn('[RTSVR6] skip overview kit: missing GLB', { full, rocks, urls });
      return null;
    }
    overviewBufCache = buf;
    overviewBufIsFull = full;
    console.log('[RTSVR6] overview kit file', {
      url: used,
      bytes: buf.byteLength,
      quest,
      rocksOnly: rocks,
...
function applyTexturePadding(THREE, root) {
  ...
  root.add(holder);
  console.log('[RTSVR6] texpad', { added: want });
}
...
export async function setupStoryKitDistanceLod(root, THREE) {
  ...
  if (root.userData && root.userData.rtsSkipDistanceLod) {
    console.log('[RTSVR6] kit distance LOD skipped (overview groundscape)');
    return;
  }
  ...
  if (lod0Root) {
    try {
      ...
      console.warn('[RTSVR6] LOD0 kit load failed, LOD2 only', err);
    }
  }
  ...
  console.log('[RTSVR6] kit distance LOD', {
...
export function applyLeanRocksHideBuildings(root) {
  ...
  updateStoryKitLodFromView();
  console.log('[RTSVR6] leanrocks: rocks shaded + building depth occluders', {
    keptBatches: kept,
...
export async function rasterizeKitHeights(root, THREE, mapSize, segsW, segsD, yieldFn) {
  ...
  console.log('[RTSVR6] kit height raster', { tris, segsW, segsD, mapSize });
  return grid;
}
