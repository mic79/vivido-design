 ...
 *
 *   node RTSVR6/scripts/bench-drive-vs-forest.mjs
 */
...
async function benchForest(context) {
  const root = path.join(WEBXR, 'rtsvr6Forest');
  const server = await startServer(root);
  const page = context.pages()[0] || (await context.newPage());
  ...
  const shot = path.join(OUT, 'rtsvr6forest-1v1.png');
  await page.screenshot({ path: shot, fullPage: false });
  const sample = await samplePage(page);
  ...
  server.close();
  await sleep(400);
  return { id: 'rtsvr6Forest', loadMs, sample, probe, screenshot: shot };
}
