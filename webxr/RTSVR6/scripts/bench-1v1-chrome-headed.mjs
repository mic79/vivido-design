 ...
 *
 *   node RTSVR6/scripts/bench-1v1-chrome-headed.mjs
 *
 * Env: SAMPLE_MS=4000
 */
...
const OUT = path.join(WEBXR, 'RTSVR6', 'bench-1v1-chrome-headed');
const SAMPLE_MS = Number(process.env.SAMPLE_MS || 4000);

const PROJECTS = [
  { id: 'RTSVR4', root: path.join(WEBXR, 'RTSVR4'), port: 8804 },
  { id: 'RTSVR6', root: path.join(WEBXR, 'RTSVR6'), port: 8805 },
  { id: 'rtsvr6Forest', root: path.join(WEBXR, 'rtsvr6Forest'), port: 8806 },
];
...
async function main() {
  ...
  const pack = {
    ...
    method: {
      harness: 'RTSVR6/scripts/bench-1v1-chrome-headed.mjs',
